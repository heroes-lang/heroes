...
pub(crate) fn foreign_word(text: &str) -> Option<(&'static str, Option<&'static str>)> {
    Some(match text {
        ...
        "fn" => ("`fn` is not a word in this language — use `function`: `function f(x: int) -> int`", Some("function")),
        ...
        "const" => ("`const` is not a word in this language — use `constant`: `constant MAX: int`", Some("constant")),
