...
**One defect is open as of 2026-09-06: 016**, below. This line said *nothing is
open* until it was filed, and it is rewritten in the same commit, which is this
file's own rule two paragraphs down.

- [ ] **016 — `assert` drops both sides for every aggregate, and the spec promises them** | 2026-09-06, found while measuring what a debugger would need to show | open, and the repair is gated on a scheduled ruling | `spec/heroes-spec.md:202` against the compiler | a silence where a message is owed, and the message is the one an author reads at the moment a test fails
    ...
    **The reproducer**, three `test` blocks in one file, run with `heroes test`:
    ...
    What it prints, measured 2026-09-06 and re-run on 2026-09-07 at `f1ebe1fc`:
...
**Defect 015 was filed and repaired the same day**, 2026-09-06, and is in
`docs/work/DONE.md`: a `certain` fix that machine-applied into a program which does
not compile, found by a panel seat outside its own remit, verified within the hour
and repaired at the class rather than at the witness — three of the ten foreign-word
swaps were broken in call position, not one.
