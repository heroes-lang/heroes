...
- [ ] **M-install-channels** | the channels, now that a release tag exists for them to pin | `site/src/html/index.html:135-137` · `seed/README.md` · `docs/ref/environment/`
    ...
    **`heroes --version` printed `heroes 0.0.1` and every tag was a milestone's
    ...
    a Docker image from the `Dockerfile` in `docs/ref/environment/linux/` — builds
    ...
    **Re-verified 2026-09-10: STILL OPEN, and its enabling fact has advanced.**
    ...
    `Dockerfile` in the tree is `docs/ref/environment/linux/`'s development box, which is
