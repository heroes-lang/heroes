...
|---|---|---|
| ffi-pragmatist | `examples/sqlite` exit 0, 0 warnings, 7 `nm` symbols, on three platforms; the width and sign refusals exit 1 on three; the legal widths exit 0 on three | **Scored on two platforms, the third has no library.** Mac: exit 0, 0 warnings, 7 `sqlite3_` undefined symbols. Debian 22 image: exit 0, 7 symbols. Windows box: no `sqlite3.h`, and the compiler says so on the author's line (`ffi_missing_header`), so the program is not measurable there. The refusals: the goldens stand on `frexp(@e: i64)` and `time(@t: u64)` rather than `getline` (the MSVC runtime has no `getline`), and both are `ffi_parameter_type` at exit 1 on all three platforms (the annotations suite, 97 green on each); `@n: u64` against `size_t *` is exit 0 on all three (`run/ffi-pointee-at-the-headers-width`). **Right in substance, wrong in the one program it named for Windows.** |
| spec-warden | `heroes measure` 3718 at the landing commit; zero false fires of the pointee check over `examples/` and `tests/golden/` by the close; `examples/sqlite` exit 0 on the Debian image | **Right on all three.** 3718 pinned by `suite_spec` from `0df0130` on; the net green over every program in `examples/` and `tests/golden/` at every step's commit (1367, 1368 and the close's run), with no `ffi_parameter_type` outside the two goldens written to fire it; sqlite exit 0 on Debian 22. |
| llm-ergonomist | silent-wrong rate on a `size_t *` pointee 25–40% under A → 0% under B; task-3 ≥ 90% right under B | **Not checkable here** — the next Part 11 harness run over FFI tasks. Stands. |
| historian | the first Apple clang rebased after 2025-09-15 rejects `uint64_t *` → `size_t *` by default | **Not checkable here** — the next Apple clang major. Stands. |
| compiler-engineer | (1)+(2)+(3)+(5) add 350–550 code_lines under `selfhost/emit/` + `selfhost/cli/`; `inst.hero` still 324; no new DECIDED entry; `examples/sqlite` exit 0 on Debian 22; cold self-build < 2% slower | **One of five wrong, and it is the size.** Measured from `c8eba69` (before step 3) to `48fc28d` (after 3c), `code_lines` over the 16 files touched under the two directories: **+840**, above the range — the pointee reader is three modules (`pointee`, `header_types`, `pointee_ask`), the arity reader a fourth, the floor a fifth, each with its module doc. `inst.hero` 324, right. No new DECIDED entry, right (two existing entries moved with dated reasons at step 1; none at step 3). sqlite exit 0 on Debian 22, right. Cold self-build: **right**: cold self-build of `selfhost/main.hero` (`rm -rf build` before each run, `/usr/bin/time -p`, alternating), old compiler (the seed before step 1, its own runtime) 61.24 s and 61.24 s, new 61.56 s and 61.59 s — **+0.5% and +0.6%**, under the 2%; the pointee check's clang runs over the compiler's own `extern` groups are what the half-second is. |
