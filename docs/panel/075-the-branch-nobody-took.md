...
What the yes settles: **candidate 2 lands** — `<`, `<=`, `>`, `>=` with an `f32`/`f64` operand abort when either side is a `nan`, while `==` and `!=` are untouched — at a measured **+27, 3440 → 3467**, spread across three places rather than one. Candidates 3 and 4 are refused on vetoes. **And the yes ratifies a correction the sitting made to itself**: the claim that this removes `sort.c`'s special case was **struck from the record**, falsified independently by three seats, two of them by deleting the guards and running.
