...
## Predictions scored at the landing commit (2026-09-03, the same evening)

| seat | prediction | result |
|---|---|---|
| compiler-engineer | the landing diff adds ≤100 non-test lines across `walk.hero`, `generics.hero`, `contextless_errors.hero` and touches no line under `selfhost/ir/` or `selfhost/emit/` beyond the two repairs | **Falsified twice, and honestly.** `git diff --stat` against `179e0768`: walk +123 (≈33 in tests), generics +119 (≈48 in tests), contextless_errors +32 — about **193 non-test lines**, of which the comments teaching the rule are the larger part (CLAUDE.md §11 wants the invariant and the why in the file). And `ir/` and `emit/` ARE touched beyond the two repairs: `containers.Function` gains `instantiated_at` and `containers.reports_at`, `mono.instantiate` takes the call span, the gate reports an instance at its call — the pragmatist's blocking condition 4 — and 32 constructor sites add the field, 20 of them in emit tests. The seat's flip-to-VETO condition — touching `mono.apply`, `phases` or `ctype`'s switch **for A itself** — is not met: `apply` changed by one comment, `phases` and `ctype` only for repairs (1) and (2). |
| compiler-engineer | the nine positions score 5 × `cannot_infer` and 4 × exit 0 | **Held**, with one correction of the seat's own table: `pair(a: ok(1), b: wanted(…))` with `pair` generic is `cannot_infer` on `ok(1)` before `wanted` is reached (the existing frontier), so the count is four no-context refusals at the call (`x = …`, `print(…)`, the UFCS receiver, `_ = …`) plus that one; `return`, the `match` arm, the annotation and `return "a".wanted()` build and run. |
| ffi-pragmatist | the two-type `empty_of` shows two instantiations in `--dump-ir` and prints `7 1 1 true`; no `_3f` in `--emit-c`; `s04`/`s05`/`s11`/`s14` exit 1 at the call | **Held on every clause**: `function empty_of(why: str) -> [i64]` and `-> [f64]` both in the dump, `s06b` prints `x`, `y`, `7 1 1 true`; `grep -c _3f` over the emitted C is 0; the four no-context probes are exit 1 `cannot_infer` at the call line and column. And the seat's condition 4 measured: `s18` (declaration above) and `s18b` (declaration below) both report `pointer_element` at the annotation's own line, `6:5` and `2:5`. |
| spec-warden, historian, ergonomist, engineer (M-corpus-depth close) | see the table above | *Not yet due.* |
