...
| Waiting on the author | **3** decisions — `panel 121`, `panel 122` and `panel 123`, the last of which retires §1.6's soft line and withdraws a floor the author was told was a fact · **46** in `SCHEDULED.md` · **1** in `DEFECTS.md` · **342** in `LEARN.md` (never a gate) · an outstanding veto (`docs/panel/101` R3) · **o200k**, whose 2026-08-26 trigger is met and whose premise panel 123 changed: the vendored spread is 78 and the real generation gap is 967 |

**Re-measured 2026-09-09 at M-cstr-lifetime's close, not carried**: the compiler's own suite is **597** and the net's own **124**. The full net last ran at **1636 passed, 4 failed** against the lend rule before its goldens moved, and every suite the change touches has been green since: `check` 99, `annotations` 135, `unsupported` 12, `emission` 429, `canonical` 1, `layout` 1, `order` 2, `fixes` 7, `records` 15, `spec` 10, `special` 10. **The full net is unrun since and runs once before a push.** Least room: `check/walk.hero` at **1697 of a decided 1700** and `check/builtins.hero` at **374 of 374**; the new `check/lending.hero` is **207** against §11's 300, which falsified a seat's ≤140 prediction and is recorded as such.
