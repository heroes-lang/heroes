...
| | |
|---|---|
| **Current milestone** | **M-c-callbacks** — **OPEN 2026-09-05**, row 34, **step 0 landed the same day**: a Heroes function reaches a C callback parameter, and a foreign thread is refused by name rather than left to corrupt (panel 111 R4 and R9, one commit because the permission is the door). Split out of M-isolated-threads by `docs/panel/111` and placed ahead of it by the author, because three seats measured that the permission is bought entirely by §1.11 and §12's FFI-completeness instruction and needs no thread at all · **M-isolated-threads** stays **OPEN 2026-09-03**, row 35, re-scoped by the same sitting: it delivers Part 7.13's isolation or it delivers nothing |
...
| Waiting on the author | **2 decisions**, re-counted 2026-09-05: whether the seed-freshness rule gets an instrument that runs on every push. The four spec silences this row named the day before — `T??`'s refusal, a `()` row in the type table, what a `constant`'s body may hold, whether a parameter list may break across lines — were settled at `docs/panel/110`, ratified 2026-09-05 by standing delegation, and are in `docs/work/DONE.md`; the list was empty for one morning and a red `main` refilled it. A **second decision** joined it at M-c-callbacks step 0: whether panel 111 R4's `ffi_callback_type` class still opens, after a measurement falsified the premise it rests on. `DEFECTS.md` is empty · **38** in `SCHEDULED.md`, every one naming an open row of the table above · **308** in `LEARN.md` (never a gate) · an **outstanding veto** of the rule that stands (`docs/panel/101` R3, *"live and unlifted"* in that sitting's own words) |
