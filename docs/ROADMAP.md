...
| | |
|---|---|
| **Current milestone** | **M-handle-verdict**, opened 2026-09-13 on the trunk and at step 1: **defect 030 is closed** — the specification stopped promising something false about what a copy is, on the one network call the author authorised the evening the ledger closed, and the shipped counterexample stopped demonstrating it. What remains is the sitting on whether one C handle can be told from another, which owns the **two** defects still open — 029 a swapped opaque handle that builds clean and segfaults, 031 one copy freeing what every other copy holds — the two things a `ptr` still does not carry: a pointee type and an ownership |
...
| Waiting on the author | **0** decisions. The eight sittings of M-deferral-ledger and the sitting-brief question were all ratified on 2026-09-13, the evening the milestone closed; panel 145 sits with its ratification not yet queued |
