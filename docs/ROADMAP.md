...
**Re-measured 2026-09-09 after `--refresh` landed, not carried**: the compiler's own suite is **605** and the net's own **125**. The full net last ran at **1636 passed, 4 failed** against the lend rule before its goldens moved, and every suite the changes since have touched has been green: `canonical` 1, `layout` 1, `order` 2, `records` 15, `spec` **11** — the new one is the record's cross-check — and `special` 10, with `check` 99, `annotations` 135, `unsupported` 12, `emission` 429 and `fixes` 7 unmoved since that close and unrun since. **The full net is unrun since and runs once before a push.** Least room: `check/walk.hero` at **1697 of a decided 1700** and `check/builtins.hero` at **374 of 374**; the new `check/lending.hero` is **207** against §11's 300, which falsified a seat's ≤140 prediction and is recorded as such.
