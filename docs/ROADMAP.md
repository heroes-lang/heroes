...
| | |
|---|---|
| **Current milestone** | **M-marked-acquisition**, opened 2026-09-14, chain row 57: the mark goes where the obligation is CREATED, on the acquiring call. Step 1 re-counted the census from the tree and found it has **two levels**; step 2 took the pricing the sitting could not, and the two instruments turned out **not to be alternatives** — escape refusal would refuse 2 of 2 of the reference binding's handle-producing wrappers, so it is a clause of the mark rather than a rival to it. The mark's surface is built and measured at **+25 lines across 7 files**; its counter is **unbuilt** |
