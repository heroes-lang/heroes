...
| | |
|---|---|
| **Current milestone** | **none open.** Next in the chain is **M-reflection-verdict**. The three sittings of 2026-09-10 and 2026-09-11 are ratified, so nothing waits on the author; **defect 026 stays open**, narrowed to the inversion a call through a function value still admits, which `heroes mutate --operator swap-args` prices at 50% unchanged, and it is what leaves the last three milestones untagged |
| **Last closed** | **M-positional-values**, 2026-09-11, untagged ([046](journal/046-positional-values.md)) — panel 128 vetoed three routes out of defect 026, each on a different seat's soundness ground, and landed the one nobody had listed · before it **M-labelled-builtins** ([045](journal/045-labelled-builtins.md)) and **M-anchored-spec** ([044](journal/044-anchored-spec.md)), both untagged · v1 **reached** at M-selfhost-fixpoint, 2026-08-18 |
| Milestones closed | **47** of 71 · **44** milestone tags, and **the last three milestones are closed untagged**, each because the one before it found the defect the next one repairs: M-anchored-spec on 025, M-labelled-builtins on 026, M-positional-values because 026 is narrowed rather than closed. **Eight rows entered on 2026-09-10 and 2026-09-11** by author decision, three of them out of the night's own findings |
...
| The spec | **5624 real tokens** on `claude-opus-5` through `count_tokens`, read by `heroes measure --refresh` at the landing, digest `222ad91e3a6b7aa3`; **4432** on the vendored ranks, which is what `SPEC_TOKENS` pins. **The ceiling is 6144** with its instrument named; **460** free net of the FFI floor. **Thirteen numbered sections since 2026-09-11** (M-anchored-spec, panel 126), each rule with one home and the numbers as the citation anchors. Panel 127 then spent one token making § 6 show the labels § 9 always required, and panel 128 twenty-five making § 3 say that a call through a function value is positional — a sentence its own warden had vetoed as false until the refusal that made it true |
| Records | sittings **128** · journals **47** · examples **120** files, **545** `test` blocks · **open defects 1**, number 026, narrowed · mutation, re-scored 2026-09-11: `swap-args` **1733 of 1910 killed, 96%** where it was 1441 and 80%, survivors **75** where they were 367, and **50%** on a call through a function value, before and after the night's work |
| Waiting on the author | **0** decisions: `panel 126`, `panel 127` and `panel 128` all ratified 2026-09-11, each conservative route recorded beside its verdict and declined · **55** in `SCHEDULED.md` · **1** in `DEFECTS.md`, number 026, narrowed · **353** in `LEARN.md` (never a gate) · an outstanding veto (`docs/panel/101` R3) · **o200k**, whose trigger is met and whose premise panel 123 changed |
...
| 48 | **M-reflection-verdict** | scheduled | — | — | the ruling on reflection — at run time, and as compile-time derivation over a record's fields — and, since 2026-09-06, on a general annotation mechanism · a decision, not a feature |
| 49 | **M-deferral-ledger** | scheduled | — | — | every Part 7 item with no milestone gets a dated verdict or a return condition |
| 50 | **M-cleanup-verdict** | scheduled | — | — | the ruling on a scope-bound release — `defer` or another form — for the two obligations `owned` and `lease` put on every path · a decision, not a feature |
| 51 | **M-check-completeness** | scheduled | — | — | what `heroes check` accepts, `heroes build` compiles — through a generic too · scheduled, no warrant
| 52 | **M-arm-platform** | scheduled | — | — | the fourth real machine: arm64 Linux, where a container runs and where `char` is unsigned · **§1.12** |
| 53 | **M-core-packages** | scheduled | — | — | small packages that compose, organised as Go's tree, in Heroes or over C |
| 54 | **M-package-manager** | scheduled | — | — | `heroes add`/`heroes fetch`; bindings instead of a standard library |
| 55 | **M-web-framework** | scheduled | — | — | composes the core packages, Go/Echo style: explicit routes, records, no magic |
| 56 | **M-doc-generator** | scheduled | — | — | `heroes doc`, the one direction Part 6's literate-source row promises · scheduled, no warrant |
| 57 | **M-panic-location** | scheduled | — | — | a panic names the `.hero` file, line and function · **§1.12** |
| 58 | **M-typed-inspection** | scheduled | — | — | a stopped program shows Heroes values: the name the author typed, and `[T]`, `{K: V}`, `T?`, a variant and a record shown as themselves · scheduled, no warrant |
| 59 | **M-generated-programs** | scheduled | — | — | programs nobody wrote: a generator that composes valid Heroes and knows the answer before the compiler is asked · **§1.12** |
| 60 | **M-thesis-harness** | scheduled | — | — | Part 11's metrics 2 and 4 run for the first time, as a Heroes program · **§1.1** |
| 61 | **M-lsp-server** | scheduled | — | — | `heroes lsp`, and the incremental frontend it needs |
| 62 | **M-vscode-extension** | scheduled | — | — | the extension, complete |
| 63 | **M-qbe-backend** | scheduled | — | — | Part 7 item 15 — the proof that the IR is not C in disguise |
| 64 | **M-journey-book** | scheduled | — | — | the journey — how this language came to be |
| 65 | **M-guide-book** | scheduled | — | — | the guide, as a book you would find in a shop · **§1.1** |
| 66 | **M-deployable-binary** | scheduled | — | — | what the machine that RUNS a Heroes program needs, on the three platforms, and which `-O` a shipped artifact carries |
| 67 | **M-microcontroller-verdict** | scheduled | — | — | the ruling on a Heroes program running on a microcontroller under an RTOS, RISC-V first; the two facts a 32-bit build refuses today are its brief · **§1.12**, Part 2 |
| 68 | **M-install-channels** | scheduled | — | — | a Homebrew tap, winget, a Nix flake, a Docker image, all built from the seed and pinned to a `v*` release tag; the version scheme itself was decided ahead, 2026-09-07 |
| 69 | **M-online-compiler** | scheduled | — | — | the compiler reached without installing anything: the site's visitor writes Heroes and gets its answer · scheduled, no warrant |
| 70 | **M-compatibility-promise** | scheduled | — | — | the paragraph `1.0.0` rests on, and the suite that makes a broken promise red · CLAUDE.md §14 |
| 71 | **M-publication-gate** | scheduled | — | — | the last gate before anything goes outward · CLAUDE.md §14 |
