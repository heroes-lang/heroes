...
| | |
|---|---|
| **Current milestone** | **M-marked-acquisition**, opened 2026-09-14, chain row 57: the mark goes where the obligation is CREATED, on the acquiring call. Step 1 re-counted the census from the tree and found it has **two levels** — the acquiring `extern` call sits one module away from every place a handle is held, so a mark must propagate through a Heroes wrapper as `consumes` did. Next is the pricing the sitting could not take, and it must be **compiled** |
