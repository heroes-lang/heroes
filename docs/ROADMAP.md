...
| `M-selfhost-nesting` | — | — | the compiler's own 169 modules move into directories by subsystem. **Scheduled 2026-09-02 by author instruction and BLOCKED on a language question the same instruction opened**: nesting by prefix family puts 32 modules into 13 last-part collision groups, which the rule `M-package-layout` landed refuses, so it waits on `docs/panel/100`'s verdict on a file-local `as` alias and inherits it. The id names the deliverable and stays true under either outcome (§14); if the sitting changes the milestone's shape rather than its method, a new id is minted and this row retires |
