...
| | |
|---|---|
| **Current milestone** | **M-interpolated-strings** — the `f"…"` literal panel 121 ruled in, opened 2026-09-09 on the verdict's close. Unblocked on the budget (panel 123's ordering veto is satisfied at `design.md:256` and `:277`); the parser seam is settled by R5 rather than by choice, the hook stays in the knot |
| **Last closed** | **M-interpolation-verdict**, 2026-09-09, `m-interpolation-verdict` ([042](journal/042-interpolation-verdict.md)) — the ruling, closed with the verdict and not the form, hours after `M-held-bytes` (`m-held-bytes`, [041](journal/041-held-bytes.md)) closed defect 024 · v1 **reached** at M-selfhost-fixpoint, 2026-08-18 |
| Milestones closed | **43** of 63 · **42** milestone tags, so one closed milestone, M-cstr-lifetime, carries none by its own design |
| The compiler | **56,690** lines of Heroes in **194** modules · the seed **768,835** lines of C at runtime ABI **22**, regenerated at the close with the fixpoint verified by `cmp` · two runtime entry points and a fourth allocation counter whose exit message accuses the program |
| The spec | **5231 real tokens** on `claude-opus-5` through `count_tokens`, read by `heroes measure --refresh` at the landing, digest `13796ab23125ab6e`; **4098** on the vendored ranks, which is what `SPEC_TOKENS` pins. **The ceiling is 6144** with its instrument named; **853** free net of the FFI floor. The lease clause cost **+103**, 32 more than the capability alone, because the two clauses that make a second release impossible are in the document |
| Records | sittings **125** · journals **43** · examples **118** files, **545** `test` blocks · open defects **0** · mutation: `drop-question` **236 of 236 killed**, `--permissive` 193, so **43** are caught only by the thesis |
| Waiting on the author | **1** decision, `panel 125` — the lease's soundness, whose conservative route (a runtime registry) is recorded beside it · **45** in `SCHEDULED.md` · **0** in `DEFECTS.md` · **349** in `LEARN.md` (never a gate) · an outstanding veto (`docs/panel/101` R3) · **o200k**, whose trigger is met and whose premise panel 123 changed |

**Re-measured 2026-09-09 at M-held-bytes' close, not carried**: the compiler's own suite is **611** and the net's own **125**. Every suite the milestone touched is green on the closing compiler: `check` 102, `annotations` 138, `fixes` 7, `run` 107, `emit` 6, `unsupported` 12, `ir` 22, `emission` 437, `corpus` 54, `warnings` 167, `determinism` 135, `lines` 107, `canonical` 1, `layout` 1, `order` 2, `spec` 11, `special` 10, `records` 15. **The full net is unrun since M-cstr-lifetime and runs once before a push, as do the Linux and Windows legs** (author instruction 2026-09-09). Least room: `check/walk.hero` at **1697 of a decided 1700**, `check/builtins.hero` at **371 of 374** after the lend typing moved out; `check/lending.hero` **234** and the new `check/leasing.hero` **199** against §11's 300.
...
| 44 | **M-interpolated-strings** | **OPEN** | — | — | the `f"…"` literal panel 121 ruled in: lexer, one AST node, checker, desugar in lowering, every printer, and the spec clause with R6's two sentences · **§1.1**
...
| 47 | **M-check-completeness** | scheduled | — | — | what `heroes check` accepts, `heroes build` compiles — through a generic too · scheduled, no warrant
| 48 | **M-core-packages** | scheduled | — | — | small packages that compose, organised as Go's tree, in Heroes or over C |
| 49 | **M-package-manager** | scheduled | — | — | `heroes add`/`heroes fetch`; bindings instead of a standard library |
| 50 | **M-web-framework** | scheduled | — | — | composes the core packages, Go/Echo style: explicit routes, records, no magic |
| 51 | **M-doc-generator** | scheduled | — | — | `heroes doc`, the one direction Part 6's literate-source row promises · scheduled, no warrant |
| 52 | **M-panic-location** | scheduled | — | — | a panic names the `.hero` file, line and function · **§1.12** |
| 53 | **M-typed-inspection** | scheduled | — | — | a stopped program shows Heroes values: the name the author typed, and `[T]`, `{K: V}`, `T?`, a variant and a record shown as themselves · scheduled, no warrant |
| 54 | **M-generated-programs** | scheduled | — | — | programs nobody wrote: a generator that composes valid Heroes and knows the answer before the compiler is asked · **§1.12** |
| 55 | **M-thesis-harness** | scheduled | — | — | Part 11's metrics 2 and 4 run for the first time, as a Heroes program · **§1.1** |
| 56 | **M-lsp-server** | scheduled | — | — | `heroes lsp`, and the incremental frontend it needs |
| 57 | **M-vscode-extension** | scheduled | — | — | the extension, complete |
| 58 | **M-qbe-backend** | scheduled | — | — | Part 7 item 15 — the proof that the IR is not C in disguise |
| 59 | **M-journey-book** | scheduled | — | — | the journey — how this language came to be |
| 60 | **M-guide-book** | scheduled | — | — | the guide, as a book you would find in a shop · **§1.1** |
| 61 | **M-install-channels** | scheduled | — | — | a Homebrew tap, winget, a Nix flake, a Docker image, all built from the seed and pinned to a `v*` release tag; the version scheme itself was decided ahead, 2026-09-07 |
| 62 | **M-online-compiler** | scheduled | — | — | the compiler reached without installing anything: the site's visitor writes Heroes and gets its answer · scheduled, no warrant |
| 63 | **M-publication-gate** | scheduled | — | — | the last gate before anything goes outward · CLAUDE.md §14 |
...
### M-cstr-lifetime — a lent string may not outlive its lender
