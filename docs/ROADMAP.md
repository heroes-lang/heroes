...
| The spec | **5094 real tokens** (Opus 5 and Sonnet 5, `count_tokens`, 2026-09-09), **4128** on Haiku 4.5 and Opus 4.6, and **3995** on the vendored ranks the checks still read — `cl100k_base` is OpenAI's and undercounts by 27.5%, so the old *"3995 of a hard 4096 with 40 free"* was never the reader's number ([023](measurements/023-the-instrument-was-not-the-readers.md)). **The ceiling is 6144 tokens** by author decision 2026-09-09, leaving **989** net of the FFI floor; the pins, `heroes measure`, the seed, the README and both home pages still carry 4096 and are a milestone the author places. The **floor** this row claimed at 4096 is withdrawn by panel 123: a minimum cacheable prefix is a property of a request and not of a file, and `CLAUDE.md` alone clears it by 1451 |
...
| 41 | **M-cstr-lifetime** | **OPEN** | — | — | a `cstr` may not outlive the `str` it was lent from: defect 022, a use-after-free that builds clean and answers wrong at exit 0 · **§1.12**
