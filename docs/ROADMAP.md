...
| | |
|---|---|
| **Current milestone** | **none open.** Next in the chain is **M-reflection-verdict**. Two ratifications are queued, `panel 126` and `panel 127`, and **defect 026** is open: a function type names no parameters, so two same-typed arguments can still be swapped in silence through a function value and through a C callback |
| **Last closed** | **M-labelled-builtins**, 2026-09-11, untagged ([045](journal/045-labelled-builtins.md)) — defect 025 repaired the evening it was filed, and the classic argument inversion fell from 367 survivors to 75 · before it **M-anchored-spec**, also untagged ([044](journal/044-anchored-spec.md)) · v1 **reached** at M-selfhost-fixpoint, 2026-08-18 |
| Milestones closed | **46** of 70 · **44** milestone tags, 32 of them `m-*` and twelve legacy, and **the last two milestones are closed untagged**: M-anchored-spec because defect 025 was open when it finished, M-labelled-builtins because defect 026 was. **Seven rows entered 2026-09-10 and 2026-09-11** by author decision, the last two out of the one before it: M-anchored-spec at 45 and M-labelled-builtins at 46, and five earlier from the chain's first reading against a written production-readiness criterion (§ What production-ready means) and from the question whether a program could run on an ESP32 |
...
| The spec | **5599 real tokens** on `claude-opus-5` through `count_tokens`, read by `heroes measure --refresh` at the landing, digest `17553bf94f6feb87`; **4410** on the vendored ranks, which is what `SPEC_TOKENS` pins. **The ceiling is 6144** with its instrument named; **485** free net of the FFI floor. **Thirteen numbered sections since 2026-09-11** (M-anchored-spec, panel 126), in the order the language Reports use, each rule with one home, and the numbers are the citation anchors: a `spec:NNN` written before that day reads against the layout at `834d804f`. The day after, panel 127 spent **one** token making § 6 show the labels § 9 always required, and paid for it by deduplicating § 11 |
| Records | sittings **127** · journals **46** · examples **120** files, **545** `test` blocks · **open defects 1**, number 026 · mutation re-scored 2026-09-11 on the repaired compiler: `swap-args` **1733 of 1910 killed, 96%**, where it was 1441 and 80%, and the survivors are **75** where they were 367; `drop-question` **236 of 236**, `--permissive` 1052 |
| Waiting on the author | **2** decisions, `panel 126` and `panel 127`, each a ratification of what already landed, with the recommendation in its queue item · **54** in `SCHEDULED.md` · **1** in `DEFECTS.md` · **353** in `LEARN.md` (never a gate) · an outstanding veto (`docs/panel/101` R3) · **o200k**, whose trigger is met and whose premise panel 123 changed |
...
One table, one row per milestone, **closed first and scheduled after**: rows 1–46
are done, in the order they closed, and rows 47–70 are what is next, in the order
...
| 47 | **M-reflection-verdict** | scheduled | — | — | the ruling on reflection — at run time, and as compile-time derivation over a record's fields — and, since 2026-09-06, on a general annotation mechanism · a decision, not a feature |
| 48 | **M-deferral-ledger** | scheduled | — | — | every Part 7 item with no milestone gets a dated verdict or a return condition |
| 49 | **M-cleanup-verdict** | scheduled | — | — | the ruling on a scope-bound release — `defer` or another form — for the two obligations `owned` and `lease` put on every path · a decision, not a feature |
| 50 | **M-check-completeness** | scheduled | — | — | what `heroes check` accepts, `heroes build` compiles — through a generic too · scheduled, no warrant
| 51 | **M-arm-platform** | scheduled | — | — | the fourth real machine: arm64 Linux, where a container runs and where `char` is unsigned · **§1.12** |
| 52 | **M-core-packages** | scheduled | — | — | small packages that compose, organised as Go's tree, in Heroes or over C |
| 53 | **M-package-manager** | scheduled | — | — | `heroes add`/`heroes fetch`; bindings instead of a standard library |
| 54 | **M-web-framework** | scheduled | — | — | composes the core packages, Go/Echo style: explicit routes, records, no magic |
| 55 | **M-doc-generator** | scheduled | — | — | `heroes doc`, the one direction Part 6's literate-source row promises · scheduled, no warrant |
| 56 | **M-panic-location** | scheduled | — | — | a panic names the `.hero` file, line and function · **§1.12** |
| 57 | **M-typed-inspection** | scheduled | — | — | a stopped program shows Heroes values: the name the author typed, and `[T]`, `{K: V}`, `T?`, a variant and a record shown as themselves · scheduled, no warrant |
| 58 | **M-generated-programs** | scheduled | — | — | programs nobody wrote: a generator that composes valid Heroes and knows the answer before the compiler is asked · **§1.12** |
| 59 | **M-thesis-harness** | scheduled | — | — | Part 11's metrics 2 and 4 run for the first time, as a Heroes program · **§1.1** |
| 60 | **M-lsp-server** | scheduled | — | — | `heroes lsp`, and the incremental frontend it needs |
| 61 | **M-vscode-extension** | scheduled | — | — | the extension, complete |
| 62 | **M-qbe-backend** | scheduled | — | — | Part 7 item 15 — the proof that the IR is not C in disguise |
| 63 | **M-journey-book** | scheduled | — | — | the journey — how this language came to be |
| 64 | **M-guide-book** | scheduled | — | — | the guide, as a book you would find in a shop · **§1.1** |
| 65 | **M-deployable-binary** | scheduled | — | — | what the machine that RUNS a Heroes program needs, on the three platforms, and which `-O` a shipped artifact carries |
| 66 | **M-microcontroller-verdict** | scheduled | — | — | the ruling on a Heroes program running on a microcontroller under an RTOS, RISC-V first; the two facts a 32-bit build refuses today are its brief · **§1.12**, Part 2 |
| 67 | **M-install-channels** | scheduled | — | — | a Homebrew tap, winget, a Nix flake, a Docker image, all built from the seed and pinned to a `v*` release tag; the version scheme itself was decided ahead, 2026-09-07 |
| 68 | **M-online-compiler** | scheduled | — | — | the compiler reached without installing anything: the site's visitor writes Heroes and gets its answer · scheduled, no warrant |
| 69 | **M-compatibility-promise** | scheduled | — | — | the paragraph `1.0.0` rests on, and the suite that makes a broken promise red · CLAUDE.md §14 |
| 70 | **M-publication-gate** | scheduled | — | — | the last gate before anything goes outward · CLAUDE.md §14 |
