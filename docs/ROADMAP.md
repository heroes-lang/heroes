...
| The compiler | **60,999** lines of Heroes in **209** modules (`find selfhost -name '*.hero'`) · the seed **813,914** lines of C, regenerated at panel 149's repair and verified at the fixpoint |
...
| Records | sittings **147** · journals **58** · milestone files **48** · entries: `docs/records/log/` **659**, `docs/records/done/` **525**, `docs/records/book/beats/` **123** · **open defects 1**, **open decisions 1** |
| Waiting on the author | **panel 149's ratification**, queued 2026-09-14, with its resolution already in force as the default. And **one thing this machine could not do**: the specification's +4 is unlanded because `heroes measure --refresh` needs `ANTHROPIC_API_KEY`, which is unset here, and the instrument refuses a verdict against a stale count |
...
the compiler's own tests **647**, `check` **118**, `run` **119**, `lines`
**120**, `warnings` **179**, `determinism` **148**, `emission` **468**,
`annotations` **154**, `fixes` **10**, `corpus` **55**, `canonical` **2**.

**Two moved and two held for the same reason.** The compiler's own tests are
**+1** and `check` **+2**, the new cases. `run` and `emission` are unchanged
