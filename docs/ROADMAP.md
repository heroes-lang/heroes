...
| | |
|---|---|
| **Current milestone** | **none open.** Next in the chain is **M-reflection-verdict**. Three ratifications are queued, `panel 126`, `panel 127` and `panel 128`, each of what already landed; **defect 026 stays open**, narrowed to the inversion a call through a function value still admits, which `heroes mutate --operator swap-args` prices at 50% unchanged |
...
| Waiting on the author | **3** decisions, `panel 126`, `panel 127` and `panel 128`, each a ratification of what already landed, with a recommendation in its queue item · **55** in `SCHEDULED.md` · **1** in `DEFECTS.md` · **353** in `LEARN.md` (never a gate) · an outstanding veto (`docs/panel/101` R3) · **o200k**, whose trigger is met and whose premise panel 123 changed |
