...
- **Bowie references belong in prose, never in the language surface.** Section headings in example
  ...
  has to parse them and by a model that has to learn them from a 10240-token spec. A cute error
...
1. **Implement first; queue the understanding.** The assistant never stops mid-step to ask.
   ...
   an entry in `docs/learn/`, read in `/learn` sessions **when the author asks and never
...
**The entire language specification — syntax, semantics, built-in library — must fit in 10240
tokens, measured by `claude-opus-5` through `POST /v1/messages/count_tokens`.** About nine pages.
...
model's own measured request offset, and the spec read **5094** on the day it was set. **Raised again to 10240 by author decision 2026-09-14**, taken while
M-marked-acquisition was landing `acquires <releaser>` and the document stood at **7921** real
against 8192 — **211 free** once §4.19's FFI floor is deducted, which is two forms' room and not
three. The instrument is unchanged and **the raise relaxes nothing**: the payment rule below is
unconditional at every level, so every row of `docs/measurements/010` is owed exactly as before.
It is **10 × 1024 and not 10000**, because the site derives this number rather than restating it
and says it in K — `site/src/lib/claims.ts`'s `ceilingK` refuses a ceiling that is not a whole
number of them, which is what keeps one number in one home. **Raised
before that to 8192 by author decision 2026-09-12**, to make room inside this document for the grammar's
...
the two signatures above are exactly the two that defect 029 was filed against, where
`sqlite3_step(db)` passed a database where a statement belonged and nothing refused it. The
handle — a fieldless `record` in a group, with a `tag` and without `partial` — is now what those
pointers are, and `examples/sqlite/main.hero` is the running witness.
...
extern "curl/curl.h" link "curl"
    ...
    record Curl tag CURL
    function curl_easy_setopt(handle: Curl, option: i64, value: cstr) -> i64
```

**`handle: ptr` until 2026-09-13**, corrected with the fence above and worth its own sentence,
because this is the binding that decided the form's SPELLING. `curl/curl.h` reads
`typedef void CURL;` on this Mac's SDK — a struct only under `CURL_STRICTER` — so there is no
`struct CURL` to name, and panel 145's completeness critic used exactly this header to correct the
`struct <tag> *` spelling every seat had proposed to the header's own name, starred. The tag is
**the author's word and the identity**: `Curl` is `CURL *`, and the emitted C says so thirteen times.
**This sentence said the tag is written *verbatim* until 2026-09-15** (panel 152 R4, defect 037),
and that was true of the emitter and false of the language: `netdb.h` declares `struct addrinfo` and
gives it no typedef, so `record AI tag addrinfo` emitted `addrinfo *` and clang refused it, and the
binding that C names with two words had no spelling at all. The emitter still writes the author's
word. Where clang refuses that word with *must use 'struct' tag to refer to type 'addrinfo'* — a
message it produces only when the bare word names a struct tag of exactly that spelling — the
compiler supplies `struct` in front of it and compiles once more, and never a name the author did
not write: a misspelled tag gets *unknown type name* from clang and stays `error[ffi_unknown_name]`.
Two routes were refused on measurement before this one was taken. A spelling read from the header's
own AST cannot disagree with the header, and `record Db tag sqlite3_stmt` gives two
`error[ffi_parameter_type]` today and zero under it (panel 151). A one-bit probe — does `struct X`
compile? — answers yes for a misspelled X too, because `struct nosuchtype *p;` is legal C (panel
152). **The falsifier, under CLAUDE.md § 12: the narrowing is wrong if any binding refused before
2026-09-15 builds under it.** `tests/golden/fixedbugs/fixedbugs-a-misspelled-handle-tag-stays-refused.hero`
is the case that would go green.
...
| Symmetric variant syntax — one spelling for building a case and matching it, as ML, Haskell, OCaml and Rust are said to have | **refused because there is no asymmetry to restore** — Part 7 item 16 until panel 142, 2026-09-13. **Nothing in this language destructures**: a record pattern is refused by name (`error[expected_pattern]`, compiled), so *build by label, read by dot* is a uniform rule and a symmetric variant pattern would be the **first destructuring form**, owing `Point(x: a, y: b)` in patterns too or leaving a new asymmetry one level up. The proposal creates the thing it removes. **Its price is §4.5**: `.circle(r: r)` binds the field's own name and shadowing is an error, so either a reader invents a second name at every pattern — and the same arm compiles or not depending on what the enclosing function binds, a **non-local intermittent** error — or patterns are exempted and an arm **silently captures** an outer name, which is a new silent class in a language whose thesis is that plausible mistakes are compile errors. **The four cited ancestors are falsified for named fields**: Haskell's omission means ⊥ building and wildcard matching, Rust's `E::V { x }` is a use in one position and a binding in the other, OCaml patterns may omit where expressions may not, and ML has no named-field data constructors at all. **Swift went the other way deliberately** (SE-0155, modified 2017-04-20: *"requiring associated value labels would be unduly onerous"*), reaching this language's asymmetry with the symmetric option in front of it; where symmetry was kept — Erlang records, Python class patterns — the documented price is that one omission means two things. §1.7's subtraction is **zero**: the two spellings are two algorithms sharing a token shape, not two copies. And it costs tokens, **+12** at best and **+67** honestly stated. **The falsifier**: a measured Part 11 first-try rate at which readers get variant construction wrong above **15%**, or a decision to admit destructuring anywhere else in the language — at which point uniformity is gone and the case for one spelling returns with it |
| `private` on a top-level declaration, hiding it from every other module | **refused because its own hinge expired and its ancestry is for another form** — Part 7 item 14 until panel 141, 2026-09-13, where it was costed a second time and no seat supported the costed form. Panel 033 deferred it in 2026-08-12 on *"one whole-program translation unit gives visibility no linkage consequence"*, and **M-separate-compilation landed 2026-08-26**: the compiler emits 157 translation units and a never-called helper takes external linkage. The premise is gone and the conclusion survives anyway, because reachability pruning already leaves a neighbouring unit zero prototypes for that helper — so what the keyword adds is `static`, which is performance, a Part 2 non-goal. §1.7's subtraction is **zero**; the cost is ~90-110 lines in 9-10 files, re-priced against `partial`, the identical shape, measured at 100 lines across 24 files. **The ancestry cited for it is for a different form**: Modula-2, Oberon, Nim, Go, Zig and Erlang all default to hidden and **none marks hiding** — four mark exporting on the declaration, one uses a list, one uses the name — so a `private` keyword on a default-visible language has no ancestor in that set; and both reversal citations are wrong as cited, Rust RFC 0001 being about struct fields. **The direction was left to a count and the count cannot decide**: three methods read 49.5%, 50% and 50% private-able of 1868 declarations, where the 42% of 2026-08 was another method on a pre-port tree in another language. **The falsifier**: M-selfhost-probe reporting a blockage that needs visibility · a measured Part 11 effect · a `build/tu-*` symbol collision the mangler cannot prevent · or a demonstration that `static` on private definitions closes a **robustness** class (§1.12) rather than a performance one, which outranks compiler size and would be funded at once. What returns instead is a rule with no keyword, named under Part 7 item 14 |
| A `raw` module, where every operation that breaks the language's guarantees would live, so a file is dangerous only by importing it (Oberon's `SYSTEM`, Modula-3's `UNSAFE`) | **refused because this document had already ruled against its premise, in this document** — Part 7 item 11 until panel 140, 2026-09-13. §4.10 names the exception in so many words, *"§4.19's `ptr`/`cstr` sits outside the guarantee"*, while Part 9 says everything breaking the guarantees goes in `raw`, **pointers first**; §1.11 decided between them before either was written, because a language with no standard library puts a pointer in **every** binding and a mark every binding carries carries no information. Measured: **56** extern declarations mention `ptr` across 37 files, while `ptr` escapes a group in a program's type position **11 times in 2 files**. **Re-measured 2026-09-14 at panel 146: the escape figure is now ZERO** — the 11 reproduces exactly at `101ab191`, the handle form took nine of them and the narrowing of `@tail` to `cstr` took the last two, so this refusal is stronger than when it was written and the row says so. The declaration figure is corrected as a **question**: its unit measures 58 across 24 files at that commit and 38 across 24 today, so the sentence's two halves were counted with different rulers and neither was written down (Part 9's own restatement of this row carries the full re-measurement). §1.7's subtraction is **zero** — the region deletes nothing, since the FFI keeps `ptr` in the core — and what it would add is core by §1.7's own test: `use X` resolves to a file on disk, so a qualified, conditional pseudo-module is a third kind of name the resolver has never had, ~100 lines across seven sites with **five of them already at their stop-on-growth ceiling**, while the operations themselves are reachable as Tier-1 builtins at **3 files**. The reader's seat found the sharper objection: `raw.load_u8` would **remove** a guarantee rather than fence a danger, since the same read through `[u8]` **aborts** at §4.13's bounds rule. And the language already implements the principle **better than the module would**: `cstr_in_a_record` and `pointer_element` confine by declaration rather than by import, cannot be renamed away as `use raw as x` could, and reach a `ptr` in a file with no `extern` group at all. **The falsifier**: a program on the §1.0 closure list, or a §4.19 ladder binding, needing a load, a store or a cast that **cannot** be written as a declaration-level builtin — so that the import line does work no annotation can do |
...
| A releaser named on a HANDLE TYPE, fired at scope exit — `record Stmt tag sqlite3_stmt released sqlite3_finalize`, Vala's `free_function` in this language's spelling | **refused because ownership is a property of the CALL and not of the type, so the mark cannot know whether this handle was given or lent** (panel 147, 2026-09-14, M-cleanup-verdict; the census is `docs/measurements/030`). **This row refuses the AXIS and not the spelling**, which is why it is here rather than deferred: the same type is handed back owned by one C function and borrowed by another, and the ffi seat compiled it — `sqlite3_db_handle` gives a `sqlite3 *` the caller must not close, `borrowed == owned: true`, and with the release in place the program printed **0 rows where the table holds 3, exit 0, ASan silent**. `sqlite3_next_stmt` is the same shape, and it is the function the milestone's own census had used. The compiler seat cast the sitting's one formal veto from the other end: the single line the sweep would add, inserted by hand into the emitted C of the shipped binding's `opened()`, made the next statement answer **`rc = 21`** — `SQLITE_MISUSE`, the connection closed before its first use — at exit 0, with the sanitizer silent because the object is freed inside system libsqlite3 while a control in the same session fired `heap-use-after-free` at 134. **So the form trades a leak for a silent wrong answer**, which §1.12 and CLAUDE.md § Precedence rank 3 make a refusal rather than a price. The spec-warden measured the yield independently and it is **zero**: 22 of the 23 escaping paths acquire through a Heroes wrapper in another module, so the scope that acquires is not the scope that leaks, and the form's own *may not be returned or stored* would refuse the reference binding at `examples/ledger/db/sqlite.hero`'s `opened` and `prepared` — write the mark and the corpus stops compiling, omit it and nothing is checked, at +80 vendored tokens. **The rule was already written and three seats reached it the expensive way**: this document has placed an FFI lifetime fact four times and never once on a type — `owned` on a RESULT, `lease` on a DECLARATION'S INITIALISER, `consumes` on a PARAMETER POSITION, the `cstr` lend on an EXPRESSION'S POSITION — and CLAUDE.md §11's *a narrowing asks the value, never the world* settles it before any C is compiled. **Precedent is real and does not save it**: Vala ships exactly this, `free_function = "sqlite3_finalize"` on `sqlite3_stmt` in the GNOME binding, alive twenty years — and Vala pays for it with `owned`/`unowned` on every reference, which is the per-call fact this row says is missing, levied everywhere instead of where it is created. **The falsifier**: a rule, compiled, by which a type-keyed releaser is suppressed for a borrowed instance **without** the binding author marking the producing call — that is, a header-derivable signal separating a function that gives ownership from one that lends it. Panel 139 measured that no such signal exists in C headers (`sqlite3_column_count` is not const-qualified, so a mutating step and a pure read have the identical signature); produce one and this row becomes a cost argument. **What this row does NOT refuse**: a scope-bound release as such. Panel 147's R4 adopts one, marked on the acquiring CALL, and it has its own milestone |
| An INFERRED release obligation — the compiler deduces that a call hands a handle over because the binding elsewhere declares a releaser for that type, so no mark is written on the acquiring call | **refused because twelve of nineteen real headers make it wrong, and the wrongness it produces is a compelled free** (panel 148, 2026-09-14, M-marked-acquisition; ratified 2026-09-14). **The count is the refusal and it was taken from headers rather than argued**: in twelve of nineteen libraries surveyed, the same library declares BOTH a releaser for a type AND a function handing back a *borrowed* instance of that same type, so an inference keyed on *this type has a releaser* marks the borrowed one too. `SSL_get0_peer_certificate` and `SSL_get1_peer_certificate` are **adjacent lines** in `openssl/ssl.h`, differ by one digit in their names, return the identical C type, and are opposite in ownership — and a probe taking both through one function type compiles with **zero warnings under `-Weverything`**, so clang cannot separate them either. The ffi seat then stopped arguing and reproduced the corruption **twice** against real libraries. **What makes this a refusal and not a price** is the direction of the error: an inference that is wrong does not omit a release, it COMPELS one the program does not own, which is §1.12's corruption rather than §1.12's leak, and CLAUDE.md § Precedence rank 3 settles it. **The falsifier, and it is the ffi seat's own words**: a rule, derivable from header text alone, separating `SSL_get0_peer_certificate` from `SSL_get1_peer_certificate`, with a measured false-acquire rate of **zero** across the twelve. Produce one and this row becomes a cost argument. **What this row does NOT refuse**: the obligation itself. It is written rather than inferred, and `acquires <releaser>` names the function that ends the life — panel 148's R2, in the language since 2026-09-14 |
| A mark (`acquires`, `borrows`, `consumes`) on a `ptr`, or on any result or parameter whose type reaches no handle | **refused because the word cannot be read there, and a word nobody reads is a comment that looks like a guarantee** (panel 150 R3, ratified 2026-09-15; `error[unread_mark]`). `check/acquiring.hero` keys its map on the handle's DECLARATION and a `ptr` has none, so `malloc(size: u64) -> ptr acquires sqlite3_notafunction` checked at exit 0 and the pointer leaked at exit 0 with zero diagnostics on every platform — the silent half of defect 037. Obeying the word was measured to be worse than ignoring it: `fopen(…) -> ptr acquires free`, released with `free`, is *attempting free on address which was not malloc()-ed* under the sanitizer, and the two `netdb.h` releasers swapped are a `SEGV` inside `freeaddrinfo`. Not widened to `ptr`, because the type system holds no pointee for one and the rule that reads the mark has nothing to key on. **What a program does instead**: name the C type the pointer stands for — `record Block tag void` is `void *`, `record AI tag addrinfo` is `struct addrinfo *` since the same day — and every mark on it is read. **The falsifier**: a program that is correct, needs the mark, and has no handle spelling. `one_tag_one_type` caps `tag void` at one per program, so two opaque `void *` families bound in one program — `malloc`/`free` beside `dlopen`/`dlclose`, say — is that program: the second family is a `ptr` with no mark, which is what it was in effect before this row. The day such a program is written and needed, this row is wrong and the row above it about the tag is where the answer would go |
...
9. ~~**Variant constructors as values** — the parser's `term` and `expression` functions are identical
   ...
   **Left this list for Part 6 at panel 138, 2026-09-13** (M-deferral-ledger step 4;
   `docs/panel/138-the-item-named-a-parser-that-never-existed-and-the-route-was-already-in-the-grammar.md`),
   refused with its falsifier in the row. The number stays 9 because Part 7's numbers are cited.
   **Every testable clause above is false, and each was measured that day.** *"The parser's `term`
   and `expression` functions"* names no parser in this repository: the compiler's expression parser
   has been precedence climbing, one `binary(@c, @a, text, min_power)`, since **2026-08-04 at 04:10**
   — the commit that first built it, the day this item was written — and `git log -S` over all
   history finds the pair only in `examples/`. It lives in **one example program written twice**,
   `examples/calculator/parse.hero:84-107` and `whole.hero:238-261`; `examples/spreadsheet/formula.hero`
   is not a third instance, since its pair returns `f64?` and builds no variant. *"Identical except
   for two names"* is **three** by one seat's count and **four** by another's, `term` appearing on
   both sides. *"Would unify into `sequence(@p, is_times, .product)`"* **would not compile even with
   the feature granted in full**, because a function type cannot carry `@` at all
   (`error[expected_type]: expected a type, found `@``; `selfhost/ast.hero:319-330` has no inout
   flag), so the sub-parser is not passable and the spelling is one parameter short. And
   *"measurably poorer"* is unearned: no language designer has published a measurement of what
   first-class constructors saved, and the only measurement in existence is this sitting's — **two
   lines per constructor against a corpus of zero**.

   **What the corpus actually wanted, compiled rather than argued**: dispatch the constructor with
   an exhaustive `match` on the value already in hand — `function joined(sep: Tok, children: [Expr])`
   returning `match sep` — which deletes both wrappers, needs no feature, and is **strictly safer**,
   because adding a case to the variant makes the dispatch `error[non_exhaustive]`, a compile error
   a function value would not have caught. **And the form that returns is a spelling already in the
   grammar**: `Expr::product` on the `::` operator, which since panel 132 already means a member
   named on its declaring type and checked against the declaration. It closes every silence the
   proposed spelling left open — `.plus` keeps the one meaning §4.5 gives it, the variant is named
   so nothing depends on the expected type, and `xs.map(Expr::number)` is decidable where
   `xs.map(.number)` is not. It is not free: `Expr::sum` is `error[variant_in_value_position]`
   today, and panel 132's own landed price for a new member node was +388/−95 across 28 files. It is
   recorded and not adopted, because no seat measured it and a sitting does not escape a measured
   refusal into an unmeasured route.
...
11. ~~**A `raw` module for low-level access** — see Part 9.~~ **Left this list for Part 6 at panel 140,
    2026-09-13** (M-deferral-ledger step 5;
    `docs/panel/140-the-document-had-already-ruled-against-part-9-and-nobody-told-part-9.md`), refused
    with its falsifier in the row. The number stays 11 because Part 7's numbers are cited. **This
    document had already ruled against Part 9's premise, in this document, and Part 9 was never
    told**: §4.10 above says *"§4.19's `ptr`/`cstr` sits outside the guarantee"* while Part 9 says
    *"everything that breaks the guarantees goes in `raw`: pointers"*, and §1.11 decided which one
    won — a language whose founding constraint is that everything comes from C cannot put pointers
    behind an import. Both of the item's stated preconditions **landed on 2026-08-12** and woke
    nobody. And the escapes it would bound are **eleven, in two files**, against **56** extern
    declarations mentioning `ptr` across 37. The correction to Part 9 itself is beneath that Part.
...
14. ~~**Declaration visibility** — `private` on a top-level declaration, hiding it from every other
    module.~~ **Left this list for Part 6 at panel 141, 2026-09-13** (M-deferral-ledger step 6;
    `docs/panel/141-the-premise-expired-the-ancestry-was-for-another-form-and-the-count-cannot-decide.md`),
    refused with its falsifier in the row. The number stays 14 because Part 7's numbers are cited.
    **Three of the paragraph below are false and are struck here.** *"One whole-program translation
    unit gives visibility no linkage consequence"* was true when it was written on 2026-08-12 and
    **stopped being true on 2026-08-26**, when M-separate-compilation landed: the compiler emits
    **157** translation units, a prototype per definition, and a never-called helper now gets
    external linkage — though the benefit is already bought without a keyword, since reachability
    pruning leaves a neighbouring unit zero prototypes for it, and what `private` would add is
    `static`, which is performance and a stated non-goal. **The ancestry is for a different form than
    the one costed**: all six languages named below default to hidden and **not one of them marks
    hiding** — Modula-2 a separate export list in a separate file, Oberon and Nim an asterisk on the
    declaration, Zig `pub`, Erlang a module attribute, Go the initial capital in the name — so a
    `private` keyword on a default-visible language has **zero** ancestry in that set, its real
    ancestry being Kotlin and C's `static`. **And both reversal citations are wrong as cited**: Rust
    RFC 0001 is about struct FIELDS rather than declaration visibility across modules, and the claim
    survives only with RFC 0026, which removed `priv`. **The two directions are also priced as one**,
    and they are not: the compiler cost is the same because the gate inverts rather than grows, but
    default-private carries **~1181 declarations** to mark and a **two-stage seed bootstrap**, the
    seed being a checked-in 796,427-line artifact that must learn to parse `export` before it can be
    regenerated over a marked tree.

    **"Direction unresolved and left to a count" is CLOSED, by measuring that the count cannot decide
    it.** Three methods ran on 2026-09-13 and read **49.5%**, **50%** and **50%** private-able of
    1868 declarations; the 42% below was another method, on a pre-port tree a third the size, in
    another language, so it never was one instrument run twice. A ratio hovering at half is not a
    tie-breaker and a fourth year of waiting would produce a fourth number near half. What could
    decide it is named instead and is runnable today: the per-file marker economy on a real
    multi-module binding, measured at **14 `export` against 33 `private`** on
    `examples/ledger/db/sqlite.hero`, once its 26 extern members — whose visibility
    `extern_across_modules` already fixes — are set aside.

    **What returns is not a keyword.** The compiler already computes the set and already owns the
    word: `HERO_TU_LOCAL` expands to `static` per module and `reach_of` already walks reachability,
    so the item's one surviving benefit — §4.4's unused rule reaching the top level — is available as
    an error class with no keyword, no AST field and no change to what `use` binds: *a top-level
    declaration that no other module names and that nothing in its own file uses is a compile
    error*. It does not record intent, which is the one thing a written marker buys, and that trade
    is written down here for the sitting that takes it. Measured beside it: Go's route is
    unavailable, because of 1868 declarations the 275 beginning with a capital are exactly the
    records, constants and variants — the case bit is spent on kind. Costed and deferred at panel 033, where five judges produced no vote to adopt. It is
...
16. **Symmetric variant syntax — v2** (panel 035's historian; author decision 2026-08-12, taken in
    ...
    **Both halves of that paragraph are falsified, and the correction is written under it rather
    ...
    that clause before it was allowed to see anything else.

    **REFUSED to Part 6 at panel 142, 2026-09-13** (M-deferral-ledger step 7;
    `docs/panel/142-there-is-no-asymmetry-to-restore-because-there-is-no-destructuring-anywhere.md`),
    and **"v2" is struck as a verdict this ledger cannot carry** — it is a promise with no date and
    no return condition, which is the shape the milestone exists to end. **What panel 133 left
    standing is false one level up: there is no asymmetry to restore, because nothing in this
    language destructures.** A record pattern is refused by name, compiled that day —
    `error[expected_pattern]: expected a pattern, found a name (`Point`)` — so **build by label, read
    by dot** holds for records exactly as for variants, and a symmetric variant pattern would be the
    **first destructuring form in the language**, owing `Point(x: a, y: b)` in patterns too or
    leaving a new asymmetry where variants destructure and records do not. **The proposal creates the
    thing it removes.** And the price of the symmetric spelling is a rule already here: `.circle(r: r)`
    asks to bind the field's own name, §4.5 forbids shadowing, and the collision compiles to
    `error[shadowed_binding]` — so either a reader invents a second name at every pattern and the
    same arm compiles or not depending on what the function above it binds, which is a non-local
    intermittent error, or patterns are exempted and an arm silently captures an outer name, which is
    a new silent class. **The four cited ancestors do not survive checking for named fields**:
    Haskell's omission means ⊥ when building and wildcard when matching, Rust's `E::V { x }` means a
    USE in an expression and a BINDING in a pattern, OCaml patterns may omit fields where expressions
    may not, and **ML has no named-field data constructors at all**. **The decisive precedent went the
    other way deliberately**: Swift's SE-0155 proposed labels required in patterns and the core team
    modified it on 2017-04-20 because *"requiring associated value labels would be unduly onerous"*,
    settling on labels mandatory at construction and omissible in patterns — this language's
    asymmetry, chosen by a committee holding the symmetric option. Symmetry is also a **cost**: +12
    for the most favourable draft, +67 for the honest one. **The falsifier**: a measured Part 11
    first-try rate at which readers get variant construction wrong above 15%, or a decision to admit
    destructuring anywhere else in the language. **What returns instead is a sentence, not a syntax**
    — the document states construction in one clause and matching in another and never states the
    contrast, and one clause at +35 would convert that silence into a rule at zero productions and
    zero migration. And the reasoning is falsified twice
...
20. **One copy of a value holding a `ptr` can free what every other copy holds, at exit 0**
    ...
    **THE CONDITION WAS MET THE SAME EVENING** (M-handle-verdict step 5, 2026-09-13). `consumes`
    on a C parameter entered with the rule *a call that ends a value's life may not be handed one
    the caller borrowed*, **114 code lines** in `selfhost/check/consuming.hero`, and the reproducer
    above is `error[consumed_borrowed_handle]` on the line that frees. **The wart STAYS, and what it
    states now is exactly the half the rule does not reach**: repaired to `closed(@h: Holder)` the
    program compiles again, a copy made BEFORE the call still holds the freed address, and
    `--sanitize` still reports `heap-use-after-free` — measured after the repair rather than
    assumed. That remaining class is the affine handle, core by §1.7 and Part 6's borrow-checker
    row's territory. **And that row's falsifier fired on this mark**: it refuses **2 of 17**
    functions in the shipped binding against the 13 of 17 the row named, so the row's ground moved
    from soundness to cost on the same day. **A cost the sitting did not price**: the mark
    PROPAGATES — `@` on two wrappers obliged every caller to hold a cell rather than a binding,
    **24 lines across `examples/ledger/`** for two marks, with output byte-identical.
...
**OVERTAKEN, corrected 2026-09-13 rather than deleted (panel 140, CLAUDE.md §14: a record is
corrected underneath).** Both preconditions landed on **2026-08-12** — modules at
M-module-namespace, sized integers at M-sized-integers — and nothing noticed, which makes this the
fourth item of M-deferral-ledger whose stated condition expired in silence. **The thesis above is
false of this language, and the reason is written two thousand lines up in this same document.**
§4.10 names the exception in so many words — *"§4.19's `ptr`/`cstr` sits outside the guarantee"* —
while the paragraph above says everything breaking the guarantees goes in `raw`, **pointers first**.
§1.11 decided between them before either was written: everything comes from C, so a pointer appears
in every binding, and a mark every binding carries carries no information. Measured 2026-09-13:
**56** extern declarations mention `ptr` across **37** files, and `ptr` escapes an `extern` group in
a program's type position **11 times in 2 files**, both of them SQLite bindings — so the region
would mark almost everything and bound almost nothing.

**Re-measured 2026-09-14 (panel 146), and the second figure is now ZERO.** The escape count of 11
reproduces exactly against `git archive 101ab191`, the last commit before the handle form: five in
`examples/sqlite/main.hero` and six in `examples/ledger/db/sqlite.hero`. After the form and after
panel 146 narrowed `@tail` from `ptr` to `cstr`, **no `ptr` escapes an `extern` group anywhere in
`examples/`**, and the eight that remain are all inside one. **The direction strengthens this
refusal rather than weakening it**: a region whose purpose is to fence the dangerous operations now
has nothing outside a group to fence. **The first figure is corrected as a QUESTION rather than as a
number**, because it could not be reproduced under any reading — the unit as this sentence states it
measures 58 declarations across 24 files at that same commit, and 38 across 24 today, while a
looser reading gives 69 across 98. The declaration count is within two and the **file count is off
by thirteen**, so the two halves of the original sentence were counted with different rulers and
neither ruler was written down. That is CL-017's shape and panel 144's own finding one sitting
earlier: **a number whose unit is not written down cannot be re-measured, so it never expires — it
just stops being checkable.**

**What a `raw` region could still have bounded is load/store at offsets and casts, and this tree
contains zero of either**, so there is nothing to move: the item was an addition wearing a move's
clothes. The panel also compiled this Part at its word — since it offers load/store on byte arrays
rather than an opaque handle type, a handle becomes an `i64` slot, and a swapped handle then exits
139 with **both sides spelled `i64`**, which is defect 029 made worse rather than better.

**The principle survives the design, and it is already implemented twice.** Heroes bounds a
dangerous type **by a per-declaration diagnostic** rather than by an import line:
`error[cstr_in_a_record]` confines `cstr`, `unsupported[pointer_element]` confines `ptr` inside
containers — no module, no `use` line, and **nothing a file can rename away**, where `use raw as x`
would let it. That shape also reaches a `ptr` in a file with **no `extern` group**, which a
file-level mark cannot. Generalising it is **M-handle-verdict's**, and the three defects open on
2026-09-13 are the three things `ptr` does not carry: a pointee type, identity, ownership.
...
instrument: if it doesn't fit in 10240 tokens, too much has been added, and you find out in an hour
