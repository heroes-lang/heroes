...
- [ ] panel 039 / DEFECT | **A `constant` is not constant, and its body is re-evaluated on every read.** `constant ARGC: int` with body `len(args())` prints `3` under `heroes run f.hero -- a b c` and `0` under `heroes run f.hero` — the same binary, two values — and a body that calls a printing function prints once per read. `ir/mod.rs:90-97` lowers a `constant` to a zero-argument function and a read to a call, which is the documented design; what nothing checks is that the body may then reach `args()`, `read_file` or `print`. It crosses §4.2's *"There are no mutable globals"* and §4.2's SCREAMING_CASE promise at once, and panel 039's llm-ergonomist **vetoed** exactly this shape on locality — a construct whose evaluation time cannot be read off its own line. Decide the restriction: a body of literals and other constants only (the ergonomist's form) · any *pure* expression · or a stated acceptance. The spec sentence is priced at **+19** and the warden refuses it until a Part 11 run shows ≥10% of models writing an illegal body, so the compiler-side rule and the spec sentence are two decisions, not one | crates/heroes/src/ir/mod.rs:90-97 · docs/panel/039 § A `constant` in Heroes is not constant | the thesis is that every plausible mistake is a compile error, and this one is a mutable global wearing SCREAMING_CASE
