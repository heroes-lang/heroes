...
- [ ] M1.3 | In Heroes today `"a\nb"` is a FOUR-character string — the backslash is an ordinary byte. How would a Heroes program express a real newline character? (Trick: today it can't) | lexer/mod.rs string() · OPEN-QUESTIONS escape item | the self-hosted lexer needs '\t' without magic numbers; panel before M6
