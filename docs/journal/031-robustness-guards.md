...
**An ABI bump is a two-phase edit, and the seed is why (step 5).** The
stamp lives in three places — the header, the emitter, the seed's eighth
line — and the seed binary on disk emits the OLD number. Bump the header
first and that binary can compile nothing, `selfhost/main.hero` included;
so the order is the emitter first (the seed still builds it, the header still
says 18), the header second (from here only the new compiler compiles a
program), the seed third (`heroes build selfhost/main.hero --emit-c -o
seed/heroes.c`, then the one clang line, then the fixpoint — the seed-built
compiler re-emitted the seed byte for byte). And everything the old stamp was
frozen into moves with it: 178 blessed emission traces and the six `emit/`
goldens, each diff read and each one exactly the ABI line. The stamp does its
job here in the other direction from panel 037's: a compiler and a runtime
from different days refuse each other at compile time, which is what made the
order of the three edits visible instead of a mystery.

**The blessing's guard refused a test, and the test was the one that was
wrong.** `is_a_capture` checks a capture's first line against the program's
own path exactly, because a capture of ANOTHER program is the race the pid
directory closes. The harness's *nothing blessed* test blessed a stub saying
`from x.hero` for a program at `<tree>/x.hero` — a header naming a different
path, which is the refused shape by construction. The stub now says the
program's own header. The other red in the same run was the old seed binary
compiling a program against a header that had moved on: exit 2, the stamp
speaking, gone the moment the seed was regenerated.

**The R5 fixture pins a silence as a rule (step 6).** Compiling a leaf below a
nested program's root re-bases every `use` in it to the leaf's directory;
with no file there it refuses and names the directory it read and the file
that decided it, which is R5's diagnostic half, already in the message since
panel 099 landed. Beside a decoy `lib/util.hero` the same leaf checks CLEAN
against a different module — the program the ergonomist built, legal under
both bases and meaning two things. The fixture holds both halves and the
rows say which is the rule and which is the hazard, so the historian's
standing prediction (Zig #13970's shape as the first defect filed) has a
place to be scored.
...
- **`UPDATE_GOLDEN` does not exist, on purpose.** The copy's first pass tried
  to refresh the six `emit/` goldens through the harness and got six failures
  and no writes: `suite_golden.hero` says the flag *is not implemented and
  must not be*, because a flag that cannot be typed is a rule that cannot be
  broken by accident. The goldens are regenerated by `--emit-c -o` on each
  case and the diff is read — six lines out, six in, all the stamp.
- **Two harness tests red in the copy, for two different reasons** (above): a
  test's own header naming the wrong path, and the seed binary's stamp against
  a header that had moved. Neither was the step's code.
