...
Moved here verbatim at `M-literal-bases` close (2026-08-12), by `/step`'s rule
that the ROADMAP says what is *next* and a closed milestone's record is its
journal — the rule that exists because that file had reached 512 lines about the
past before its first line about the future. The wording is the milestone's own
and is not rewritten; `Next:` names what was next **then**.
