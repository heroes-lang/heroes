...
| `checksum/` | CRC-32, Adler-32 and Base64, in four modules, each checked against the document that defines it — the CRC catalogue's `check=0xcbf43926`, RFC 1950's two sums, and RFC 4648 § 10's vectors both ways. The **printed output is RFC 4648 § 10's own BASE64 table**, `diff`-identical to the lines fetched from `rfc-editor.org`. It is also where the corpus first declares `u8`, `u16`, `u32` and `u64` in types instead of converting through them, because the bitwise operators are `i64` only, so a 32-bit register cannot exist and the width lives at the boundary. Writing the decoder found a language fact: **nothing in Heroes turns a byte value into a character**, so `decoded` gives back a `[u8]` — the witness for M-core-packages' byte-buffer question |
| `collatz/` | the longest Collatz chain below a bound, split across threads, and the lesson is that the bands do NOT take the same time: equal ranges are not equal work |
...
| `dotproduct/` | a dot product of two long vectors, banded across threads: the simplest shape in the thread family, and the baseline the others are read against |
...
| `firsthit/` | the first position in a long range where a condition holds, searched by several threads, where the reduction is a minimum rather than a sum and the minimum has to be the FIRST |
| `floats/` | `inf` and `nan`, and the one comparison that survives them (`x != x`) |
| `histogram/` | how often each digit appears, one thread per BUCKET rather than one per band: the member of the family that partitions the answer instead of the input, so no two threads write one place |
| `ini/` | a configuration file into `{str: {str: str}}`, built from text, with four named refusals |
| `interpreter/` | a whole little language in **eleven modules across four directories** — scan, parse, run — and the corpus's first program of the compiler's own shape. It is where four language facts are visible in working code rather than in a diagnostic: a module cycle is refused, so the recursion between statements and expressions crosses the boundary as a **function value**; a function type cannot carry an `@`, so the interpreter's whole state is passed by value and handed back; `syn/expr` and `run/expr` are two modules with one noun, so one of them is renamed at its `use`; and `util/each`'s generics are instantiated at `str` from `syn/` and at `Value` from `run/`, which the corpus had never done across a `use`. Every way a script could kill it is a named refusal — division by zero, `%` by zero, overflow on `+ - *`, a number too large to scan, a runaway recursion — because a script is input. Writing it found two compiler defects (defect 006 in `docs/work/DONE.md`, `007`), one of them `heroes check` exiting 139 in silence on a valid program |
...
| `ledger/` | a book of accounts kept in **SQLite at program scale** — four modules, eighteen bound functions, one prepared statement reused across ten inserts inside a transaction, three column types read through a `variant` that cannot forget the fifth one SQLite has, and a join. The corpus binds SQLite twice on purpose: `sqlite/` asks *does the architecture work at all* and this one asks *does it hold when a program leans on it*. **Every number is computed twice by routes that share no line** — SQLite's `sum … group by` against the same total in Heroes — because nobody has published the answer for a book somebody invented. Money is an integer number of cents and never an `f64`, and the one float in it, SQLite's `avg`, is compared with a tolerance that says so |
...
| `mandelbrot/` | how many iterations a band of the Mandelbrot set needs, counted by several threads: the two-dimensional member, and the bands cost what the set makes them cost |
...
| `matmul/` | two matrices multiplied, a band of rows per thread: the member where a thread builds a big structure and may hand back only a number, so what it hands back is a checksum of its own band |
...
| `montecarlo/` | pi by counting darts, one thread per SEED rather than per stream, because two threads sharing a generator is the classic race and here it cannot be written |
| `nbody/` | five bodies under gravity for a thousand steps, and the two energies the benchmarks game publishes — `-0.169075164` before, `-0.169087605` after. The initial conditions are copied digit for digit from the reference C program, written without exponents because this language has none. The first program here where the FFI is **inside** a real program: it cannot take a step without `sqrt` |
| `nqueens/` | how many ways eight queens fit, one thread per opening move: the member whose work is a search TREE rather than a range, so the threads finish at wildly different times |
...
| `query/` | a relational engine over a table it writes itself: a deterministic generator makes **5,000 rows**, the program writes them to a file, reads them back through a strict reader and asserts the round trip, then filters, joins, merge-sorts and groups them whole. The corpus's first program to hand the language data at volume — every other one reads an input of at most 28 lines — so it is where copy-on-write costs something, under `--sanitize` like everything else. Also the first `sort` on a `bool` and the first **map keyed by a record**. A query here is a **value** and not a string: the corpus already had three parsers, and a `variant` used as an instruction set was a shape it did not have. Every number it prints is computed twice by routes that share no line, because there is no published answer for a table a program invented |
...
| `spectral/` | the largest singular value of a matrix **nobody stores** — every entry is computed from its two indices — against the benchmarks game's published `1.274219991` at N = 100. The index arithmetic stays whole-number, because doing it in `f64` would give a different matrix and a plausible wrong answer: the kind of mistake a published number catches and a self-written assertion does not |
...
| `tally/` | counting what arrives on standard input, through `getchar`: the corpus's only program that is FED rather than told where to look, and the only one carrying a `main.stdin` |
...
| `threads/` | threads, and what may cross into one: only an `i64`, in either direction (design.md Part 7.13). The rule the nine programs above and below it rest on |
...
| `wordbands/` | counting words in text each thread writes for itself: the member that shows a thread using the parts of the language people expect to be off limits inside one, strings and arrays and maps, all values that copy |
...
**And this directory is now what the site shows.** Every program here is a page
on heroes-lang.org, in both editions, generated from these files when the site
is built: the code, the exact `heroes run` line rebuilt from `main.args` and
`main.stdin`, and the output out of `main.expected`. A program joins the site by
existing, the same way it joins the suite, and the site's build refuses to ship
if a program has no description written for it. So adding a directory here is
also adding a page, and `site/README.md` § the examples browser is where the
