/* Generated by heroes 0.0.1 from tests/golden/fixedbugs/ffi-out-parameter-guard.hero. Do not edit. */
#include "heroes_runtime.h"
#include <math.h>
#include <hero_os.h>
#include <sqlite3.h>

_Static_assert(HERO_RUNTIME_ABI == 14, "heroes_runtime.h is from another compiler");

#define HERO_RET_INT(c) _Generic((c), signed char:1, short:1, int:1, long:1, long long:1, unsigned char:1, unsigned short:1, unsigned int:1, default:0)
#define HERO_C_INTEGER(c) _Generic((c), _Bool:1, char:1, signed char:1, short:1, int:1, long:1, long long:1, unsigned char:1, unsigned short:1, unsigned int:1, unsigned long:1, unsigned long long:1, default:0)
#define HERO_C_UNSIGNED(c) _Generic((c), unsigned char:1, unsigned short:1, unsigned int:1, unsigned long:1, unsigned long long:1, _Bool:1, char:((char)-1 > 0), signed char:0, short:0, int:0, long:0, long long:0, default:0)
#define HERO_RET_I8(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 1)
#define HERO_RET_I16(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 2)
#define HERO_RET_I32(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 4)
#define HERO_RET_U8(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 1)
#define HERO_RET_U16(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 2)
#define HERO_RET_U32(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 4)
#define HERO_RET_U64(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 8)
#define HERO_RET_F32(c) _Generic((c), float:1, default:0)
#define HERO_RET_F64(c) _Generic((c), float:1, double:1, long double:1, default:0)
#define HERO_RET_BOOL(c) _Generic((c), _Bool:1, default:0)
#define HERO_RET_STR(c) _Generic((c), HeroStr:1, default:0)
#define HERO_RET_UNIT(c) __builtin_types_compatible_p(__typeof__(c), void)
#define HERO_RET_PTR(c) (__builtin_classify_type(c) == 5)
_Static_assert(HERO_RET_INT(SQLITE_OK), "heroes-ffi-return SQLITE_OK i64");
_Static_assert(__builtin_constant_p(SQLITE_OK), "heroes-ffi-const SQLITE_OK");
_Static_assert(HERO_RET_INT(sqlite3_open(0, (void * *)0)), "heroes-ffi-return sqlite3_open i64");
_Static_assert(HERO_RET_INT(sqlite3_prepare_v2((void *)0, 0, (int32_t)0, (void * *)0, 0)), "heroes-ffi-return sqlite3_prepare_v2 i64");
_Static_assert(HERO_RET_INT(HERO_OS_OK), "heroes-ffi-return HERO_OS_OK i64");
_Static_assert(__builtin_constant_p(HERO_OS_OK), "heroes-ffi-const HERO_OS_OK");
_Static_assert(HERO_RET_INT(HERO_OS_NOT_FOUND), "heroes-ffi-return HERO_OS_NOT_FOUND i64");
_Static_assert(__builtin_constant_p(HERO_OS_NOT_FOUND), "heroes-ffi-const HERO_OS_NOT_FOUND");
_Static_assert(HERO_RET_STR(hero_file_read(0, (int64_t *)0)), "heroes-ffi-return hero_file_read str");
_Static_assert(HERO_RET_INT(hero_file_write(0, (HeroStr){0})), "heroes-ffi-return hero_file_write i64");
_Static_assert(HERO_RET_INT(hero_args_count()), "heroes-ffi-return hero_args_count i64");
_Static_assert(HERO_RET_STR(hero_args_at((int64_t)0)), "heroes-ffi-return hero_args_at str");
_Static_assert(HERO_RET_UNIT(hero_exit((int64_t)0)), "heroes-ffi-return hero_exit ()");

#line 35 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
__attribute__((unused)) static void hero_ffi_probe_h_ffioutparameterguard_sqlite3_open(const char * a0, void * * a1) { (void)sqlite3_open(a0, a1); }
#line 36 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
__attribute__((unused)) static void hero_ffi_probe_h_ffioutparameterguard_sqlite3_prepare_v2(void * a0, const char * a1, int32_t a2, void * * a3, const char * * a4) { (void)sqlite3_prepare_v2(a0, a1, a2, a3, a4); }
#line 111 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_file_read(const char * a0, int64_t * a1) { (void)hero_file_read(a0, a1); }
#line 112 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_file_write(const char * a0, HeroStr a1) { (void)hero_file_write(a0, a1); }
#line 114 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_args_at(int64_t a0) { (void)hero_args_at(a0); }
#line 115 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_exit(int64_t a0) { (void)hero_exit(a0); }
#line 52 "ffioutparameterguard.c"

HERO_STR_STATIC(hero_str_0, ":memory:");
HERO_STR_STATIC(hero_str_1, "no database");
HERO_STR_STATIC(hero_str_2, "select 1");

typedef struct h_ffioutparameterguard_0opt0 {
    int64_t tag;
    union {
        HeroStr ok;
        HeroFailure err;
    } as;
} h_ffioutparameterguard_0opt0;

typedef struct h_ffioutparameterguard_0opt1 {
    int64_t tag;
    union {
        HeroFailure err;
    } as;
} h_ffioutparameterguard_0opt1;

void h_ffioutparameterguard_0opt0_retain(const h_ffioutparameterguard_0opt0 *v);
void h_ffioutparameterguard_0opt0_release(h_ffioutparameterguard_0opt0 *v);
bool h_ffioutparameterguard_0opt0_eq(const h_ffioutparameterguard_0opt0 *a, const h_ffioutparameterguard_0opt0 *b);
uint64_t h_ffioutparameterguard_0opt0_hash(const void *elem);
void h_ffioutparameterguard_0opt1_retain(const h_ffioutparameterguard_0opt1 *v);
void h_ffioutparameterguard_0opt1_release(h_ffioutparameterguard_0opt1 *v);
bool h_ffioutparameterguard_0opt1_eq(const h_ffioutparameterguard_0opt1 *a, const h_ffioutparameterguard_0opt1 *b);
uint64_t h_ffioutparameterguard_0opt1_hash(const void *elem);

int64_t h_ffioutparameterguard_SQLITE_OK(void);
void h_ffioutparameterguard_main(void);

#line 34 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
int64_t h_ffioutparameterguard_SQLITE_OK(void) {
#line 87 "ffioutparameterguard.c"
    return SQLITE_OK;
}

#line 38 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
void h_ffioutparameterguard_main(void) {
#line 93 "ffioutparameterguard.c"
    void * h0_db;
    __attribute__((unused)) void * h1_stmt;
    __attribute__((unused)) const char * h2_tail;
    int64_t h3_rc;
    void * t1;
    HeroStr t2 = {0};
    const char * t3;
    int64_t t4;
    int64_t t5;
    bool t6;
    HeroStr t7 = {0};
    void * t8;
    const char * t9;
    void * t10;
    HeroStr t11 = {0};
    const char * t12;
    int32_t t13;
    int64_t t14;
    int64_t t15;
    int64_t t16;
    bool t17;
    goto bb0;
bb0:
#line 39 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t1 = ((void *)0);
#line 39 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    h0_db = t1;
#line 40 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t2 = HERO_STR_LIT(hero_str_0);
#line 40 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t3 = hero_str_cstr(t2);
#line 40 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t4 = sqlite3_open(hero_cstr_nonnull(t3), &h0_db);
#line 40 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t5 = h_ffioutparameterguard_SQLITE_OK();
#line 40 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t6 = t4 != t5;
#line 40 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    if (t6) goto bb2; else goto bb3;
#line 40 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
bb1:
#line 43 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t8 = ((void *)0);
#line 43 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    h1_stmt = t8;
#line 44 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t9 = ((void *)0);
#line 44 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    h2_tail = t9;
#line 45 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t10 = h0_db;
#line 45 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t11 = HERO_STR_LIT(hero_str_2);
#line 45 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t12 = hero_str_cstr(t11);
#line 45 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t13 = INT64_C(-1);
#line 45 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t14 = sqlite3_prepare_v2(t10, hero_cstr_nonnull(t12), t13, &h1_stmt, &h2_tail);
#line 45 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    h3_rc = t14;
#line 46 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t15 = h3_rc;
#line 46 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t16 = h_ffioutparameterguard_SQLITE_OK();
#line 46 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t17 = t15 == t16;
#line 46 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    hero_print_bool(t17);
#line 46 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    hero_print_end();
#line 46 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    return;
#line 46 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
bb2:
#line 41 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    t7 = HERO_STR_LIT(hero_str_1);
#line 41 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    hero_print_str(t7);
#line 41 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    hero_print_end();
#line 41 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    return;
#line 41 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
bb3:
#line 41 "tests/golden/fixedbugs/ffi-out-parameter-guard.hero"
    goto bb1;
#line 181 "ffioutparameterguard.c"
}
void h_ffioutparameterguard_0opt0_retain(const h_ffioutparameterguard_0opt0 *v) {
    if (v->tag == INT64_C(0)) {
        hero_str_incref(v->as.ok);
    } else {
        hero_failure_retain(&v->as.err);
    }
}

void h_ffioutparameterguard_0opt0_release(h_ffioutparameterguard_0opt0 *v) {
    if (v->tag == INT64_C(0)) {
        hero_str_decref(v->as.ok);
    } else {
        hero_failure_release(&v->as.err);
    }
}

bool h_ffioutparameterguard_0opt0_eq(const h_ffioutparameterguard_0opt0 *a, const h_ffioutparameterguard_0opt0 *b) {
    if (a->tag != b->tag) return false;
    if (a->tag != INT64_C(0)) return hero_failure_eq(&a->as.err, &b->as.err);
    return (&hero_desc_str)->eq(&a->as.ok, &b->as.ok);
}

uint64_t h_ffioutparameterguard_0opt0_hash(const void *elem) {
    const h_ffioutparameterguard_0opt0 *v = elem;
    uint64_t h = (UINT64_C(0xcbf29ce484222325) ^ (uint64_t)v->tag)
        * UINT64_C(0x100000001b3);
    if (v->tag != INT64_C(0)) {
        return (h ^ hero_failure_hash(&v->as.err)) * UINT64_C(0x100000001b3);
    }
    return (h ^ (&hero_desc_str)->hash(&v->as.ok)) * UINT64_C(0x100000001b3);
}

void h_ffioutparameterguard_0opt1_retain(const h_ffioutparameterguard_0opt1 *v) {
    if (v->tag == INT64_C(0)) {
        return;
    } else {
        hero_failure_retain(&v->as.err);
    }
}

void h_ffioutparameterguard_0opt1_release(h_ffioutparameterguard_0opt1 *v) {
    if (v->tag == INT64_C(0)) {
        return;
    } else {
        hero_failure_release(&v->as.err);
    }
}

bool h_ffioutparameterguard_0opt1_eq(const h_ffioutparameterguard_0opt1 *a, const h_ffioutparameterguard_0opt1 *b) {
    if (a->tag != b->tag) return false;
    if (a->tag != INT64_C(0)) return hero_failure_eq(&a->as.err, &b->as.err);
    return true;
}

uint64_t h_ffioutparameterguard_0opt1_hash(const void *elem) {
    const h_ffioutparameterguard_0opt1 *v = elem;
    uint64_t h = (UINT64_C(0xcbf29ce484222325) ^ (uint64_t)v->tag)
        * UINT64_C(0x100000001b3);
    if (v->tag != INT64_C(0)) {
        return (h ^ hero_failure_hash(&v->as.err)) * UINT64_C(0x100000001b3);
    }
    return h;
}


int main(int argc, char **argv) {
    hero_args_set(argc, argv);
    h_ffioutparameterguard_main();
    hero_runtime_check_leaks();
    return 0;
}
