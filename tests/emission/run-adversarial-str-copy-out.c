/* Generated by heroes 0.0.1 from tests/golden/run/adversarial-str-copy-out.hero. Do not edit. */
#include "heroes_runtime.h"
#include <math.h>
#include <hero_os.h>
#include <heroes_runtime.h>

_Static_assert(HERO_RUNTIME_ABI == 15, "heroes_runtime.h is from another compiler");

#define HERO_RET_INT(c) _Generic((c), signed char:1, short:1, int:1, long:1, long long:1, unsigned char:1, unsigned short:1, unsigned int:1, default:0)
#define HERO_C_INTEGER(c) _Generic((c), _Bool:1, char:1, signed char:1, short:1, int:1, long:1, long long:1, unsigned char:1, unsigned short:1, unsigned int:1, unsigned long:1, unsigned long long:1, default:0)
#define HERO_C_UNSIGNED(c) _Generic((c), unsigned char:1, unsigned short:1, unsigned int:1, unsigned long:1, unsigned long long:1, _Bool:1, char:((char)-1 > 0), signed char:0, short:0, int:0, long:0, long long:0, default:0)
#define HERO_RET_I8(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 1)
#define HERO_RET_I16(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 2)
#define HERO_RET_I32(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 4)
#define HERO_RET_U8(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 1)
#define HERO_RET_U16(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 2)
#define HERO_RET_U32(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 4)
#define HERO_RET_U64(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 8)
#define HERO_RET_F32(c) _Generic((c), float:1, default:0)
#define HERO_RET_F64(c) _Generic((c), float:1, double:1, long double:1, default:0)
#define HERO_RET_BOOL(c) _Generic((c), _Bool:1, default:0)
#define HERO_RET_STR(c) _Generic((c), HeroStr:1, default:0)
#define HERO_RET_UNIT(c) __builtin_types_compatible_p(__typeof__(c), void)
#define HERO_RET_PTR(c) (__builtin_classify_type(c) == 5)
#define HERO_RET_CSTR(c) _Generic((c), char *:1, const char *:1, signed char *:1, const signed char *:1, unsigned char *:1, const unsigned char *:1, default:0)
_Static_assert(HERO_RET_INT(HERO_OS_OK), "heroes-ffi-return HERO_OS_OK i64");
_Static_assert(__builtin_constant_p(HERO_OS_OK), "heroes-ffi-const HERO_OS_OK");
_Static_assert(HERO_RET_INT(HERO_OS_NOT_FOUND), "heroes-ffi-return HERO_OS_NOT_FOUND i64");
_Static_assert(__builtin_constant_p(HERO_OS_NOT_FOUND), "heroes-ffi-const HERO_OS_NOT_FOUND");
_Static_assert(HERO_RET_STR(hero_file_read(0, (int64_t *)0)), "heroes-ffi-return hero_file_read str");
_Static_assert(HERO_RET_INT(hero_file_write(0, (HeroStr){0})), "heroes-ffi-return hero_file_write i64");
_Static_assert(HERO_RET_INT(hero_args_count()), "heroes-ffi-return hero_args_count i64");
_Static_assert(HERO_RET_STR(hero_args_at((int64_t)0)), "heroes-ffi-return hero_args_at str");
_Static_assert(HERO_RET_CSTR(hero_args_raw((int64_t)0)), "heroes-ffi-return hero_args_raw cstr");
_Static_assert(HERO_RET_UNIT(hero_exit((int64_t)0)), "heroes-ffi-return hero_exit ()");
_Static_assert(HERO_RET_INT(HERO_STR_OK), "heroes-ffi-return HERO_STR_OK i64");
_Static_assert(__builtin_constant_p(HERO_STR_OK), "heroes-ffi-const HERO_STR_OK");
_Static_assert(HERO_RET_STR(hero_str_try_from_cstr(0, (int64_t *)0)), "heroes-ffi-return hero_str_try_from_cstr str");

#line 111 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_file_read(const char * a0, int64_t * a1) { (void)(hero_file_read)(a0, a1); }
#line 112 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_file_write(const char * a0, HeroStr a1) { (void)(hero_file_write)(a0, a1); }
#line 114 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_args_at(int64_t a0) { (void)(hero_args_at)(a0); }
#line 115 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_args_raw(int64_t a0) { (void)(hero_args_raw)(a0); }
#line 116 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_exit(int64_t a0) { (void)(hero_exit)(a0); }
#line 122 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_str_try_from_cstr(const char * a0, int64_t * a1) { (void)(hero_str_try_from_cstr)(a0, a1); }
#line 53 "adversarialstrcopyout.c"

HERO_STR_STATIC(hero_str_0, "!");
HERO_STR_STATIC(hero_str_1, "hey");
HERO_STR_STATIC(hero_str_2, "x");

typedef struct h_adversarialstrcopyout_0opt0 {
    int64_t tag;
    union {
        HeroStr ok;
        HeroFailure err;
    } as;
} h_adversarialstrcopyout_0opt0;

typedef struct h_adversarialstrcopyout_0opt1 {
    int64_t tag;
    union {
        HeroFailure err;
    } as;
} h_adversarialstrcopyout_0opt1;

void h_adversarialstrcopyout_0opt0_retain(const h_adversarialstrcopyout_0opt0 *v);
void h_adversarialstrcopyout_0opt0_release(h_adversarialstrcopyout_0opt0 *v);
bool h_adversarialstrcopyout_0opt0_eq(const h_adversarialstrcopyout_0opt0 *a, const h_adversarialstrcopyout_0opt0 *b);
uint64_t h_adversarialstrcopyout_0opt0_hash(const void *elem);
void h_adversarialstrcopyout_0opt1_retain(const h_adversarialstrcopyout_0opt1 *v);
void h_adversarialstrcopyout_0opt1_release(h_adversarialstrcopyout_0opt1 *v);
bool h_adversarialstrcopyout_0opt1_eq(const h_adversarialstrcopyout_0opt1 *a, const h_adversarialstrcopyout_0opt1 *b);
uint64_t h_adversarialstrcopyout_0opt1_hash(const void *elem);

void h_adversarialstrcopyout_extend(HeroStr *ph0_s, int64_t h1_times);
void h_adversarialstrcopyout_main(void);

#line 13 "tests/golden/run/adversarial-str-copy-out.hero"
void h_adversarialstrcopyout_extend(HeroStr *ph0_s, int64_t h1_times) {
#line 88 "adversarialstrcopyout.c"
    HeroStr h0_s = {0};
    int64_t h2_i;
    HeroStr h3_own3 = {0};
    int64_t t1;
    int64_t t2;
    int64_t t3;
    bool t4;
    HeroStr t5 = {0};
    HeroStr t6 = {0};
    HeroStr t7 = {0};
    int64_t t8;
    int64_t t9;
    int64_t t10;
    int64_t t11;
    int64_t t12;
    bool t13;
    HeroStr t14 = {0};
    HeroStr t15 = {0};
    HeroStr t16 = {0};
    HeroStr t17 = {0};
    h0_s = *ph0_s;
    goto bb0;
bb0:
#line 14 "tests/golden/run/adversarial-str-copy-out.hero"
    t1 = INT64_C(0);
#line 14 "tests/golden/run/adversarial-str-copy-out.hero"
    h2_i = t1;
#line 14 "tests/golden/run/adversarial-str-copy-out.hero"
    goto bb1;
#line 14 "tests/golden/run/adversarial-str-copy-out.hero"
bb1:
#line 15 "tests/golden/run/adversarial-str-copy-out.hero"
    t2 = h2_i;
#line 15 "tests/golden/run/adversarial-str-copy-out.hero"
    t3 = h1_times;
#line 15 "tests/golden/run/adversarial-str-copy-out.hero"
    t4 = t2 < t3;
#line 15 "tests/golden/run/adversarial-str-copy-out.hero"
    if (t4) goto bb2; else goto bb3;
#line 15 "tests/golden/run/adversarial-str-copy-out.hero"
bb2:
#line 16 "tests/golden/run/adversarial-str-copy-out.hero"
    t5 = h0_s;
#line 16 "tests/golden/run/adversarial-str-copy-out.hero"
    t6 = HERO_STR_LIT(hero_str_0);
#line 16 "tests/golden/run/adversarial-str-copy-out.hero"
    t7 = hero_str_concat(t5, t6);
#line 16 "tests/golden/run/adversarial-str-copy-out.hero"
    t14 = h3_own3;
#line 16 "tests/golden/run/adversarial-str-copy-out.hero"
    h3_own3 = t7;
#line 140 "adversarialstrcopyout.c"
    hero_str_decref(t14);
#line 16 "tests/golden/run/adversarial-str-copy-out.hero"
    t15 = h0_s;
#line 144 "adversarialstrcopyout.c"
    hero_str_incref(t7);
#line 16 "tests/golden/run/adversarial-str-copy-out.hero"
    h0_s = t7;
#line 148 "adversarialstrcopyout.c"
    hero_str_decref(t15);
#line 17 "tests/golden/run/adversarial-str-copy-out.hero"
    t8 = h2_i;
#line 17 "tests/golden/run/adversarial-str-copy-out.hero"
    t9 = INT64_C(1);
#line 17 "tests/golden/run/adversarial-str-copy-out.hero"
    if (__builtin_add_overflow(t8, t9, &t10)) hero_panic_overflow();
#line 17 "tests/golden/run/adversarial-str-copy-out.hero"
    h2_i = t10;
#line 18 "tests/golden/run/adversarial-str-copy-out.hero"
    t11 = h2_i;
#line 18 "tests/golden/run/adversarial-str-copy-out.hero"
    t12 = INT64_C(2);
#line 18 "tests/golden/run/adversarial-str-copy-out.hero"
    t13 = t11 == t12;
#line 18 "tests/golden/run/adversarial-str-copy-out.hero"
    if (t13) goto bb5; else goto bb6;
#line 18 "tests/golden/run/adversarial-str-copy-out.hero"
bb3:
#line 168 "adversarialstrcopyout.c"
    *ph0_s = h0_s;
#line 13 "tests/golden/run/adversarial-str-copy-out.hero"
    t16 = h3_own3;
#line 172 "adversarialstrcopyout.c"
    hero_str_decref(t16);
    return;
bb4:
    goto bb1;
bb5:
    *ph0_s = h0_s;
#line 13 "tests/golden/run/adversarial-str-copy-out.hero"
    t17 = h3_own3;
#line 181 "adversarialstrcopyout.c"
    hero_str_decref(t17);
    return;
bb6:
    goto bb4;
}

#line 21 "tests/golden/run/adversarial-str-copy-out.hero"
void h_adversarialstrcopyout_main(void) {
#line 190 "adversarialstrcopyout.c"
    HeroStr h0_a = {0};
    HeroStr h1_b = {0};
    HeroStr t1 = {0};
    int64_t t2;
    HeroStr t3 = {0};
    HeroStr t4 = {0};
    int64_t t5;
    HeroStr t6 = {0};
    int64_t t7;
    HeroStr t8 = {0};
    HeroStr t9 = {0};
    int64_t t10;
    HeroStr t11 = {0};
    HeroStr t12 = {0};
    HeroStr t13 = {0};
    HeroStr t14 = {0};
    goto bb0;
bb0:
#line 22 "tests/golden/run/adversarial-str-copy-out.hero"
    t1 = HERO_STR_LIT(hero_str_1);
#line 22 "tests/golden/run/adversarial-str-copy-out.hero"
    t11 = h0_a;
#line 213 "adversarialstrcopyout.c"
    hero_str_incref(t1);
#line 22 "tests/golden/run/adversarial-str-copy-out.hero"
    h0_a = t1;
#line 217 "adversarialstrcopyout.c"
    hero_str_decref(t11);
#line 23 "tests/golden/run/adversarial-str-copy-out.hero"
    t2 = INT64_C(5);
#line 23 "tests/golden/run/adversarial-str-copy-out.hero"
    h_adversarialstrcopyout_extend(&h0_a, t2);
#line 24 "tests/golden/run/adversarial-str-copy-out.hero"
    t3 = h0_a;
#line 24 "tests/golden/run/adversarial-str-copy-out.hero"
    hero_print_str(t3);
#line 24 "tests/golden/run/adversarial-str-copy-out.hero"
    hero_print_end();
#line 25 "tests/golden/run/adversarial-str-copy-out.hero"
    t4 = h0_a;
#line 25 "tests/golden/run/adversarial-str-copy-out.hero"
    t5 = hero_str_len(t4);
#line 25 "tests/golden/run/adversarial-str-copy-out.hero"
    hero_print_int(t5);
#line 25 "tests/golden/run/adversarial-str-copy-out.hero"
    hero_print_end();
#line 26 "tests/golden/run/adversarial-str-copy-out.hero"
    t6 = HERO_STR_LIT(hero_str_2);
#line 26 "tests/golden/run/adversarial-str-copy-out.hero"
    t12 = h1_b;
#line 241 "adversarialstrcopyout.c"
    hero_str_incref(t6);
#line 26 "tests/golden/run/adversarial-str-copy-out.hero"
    h1_b = t6;
#line 245 "adversarialstrcopyout.c"
    hero_str_decref(t12);
#line 27 "tests/golden/run/adversarial-str-copy-out.hero"
    t7 = INT64_C(0);
#line 27 "tests/golden/run/adversarial-str-copy-out.hero"
    h_adversarialstrcopyout_extend(&h1_b, t7);
#line 28 "tests/golden/run/adversarial-str-copy-out.hero"
    t8 = h1_b;
#line 28 "tests/golden/run/adversarial-str-copy-out.hero"
    hero_print_str(t8);
#line 28 "tests/golden/run/adversarial-str-copy-out.hero"
    hero_print_end();
#line 29 "tests/golden/run/adversarial-str-copy-out.hero"
    t9 = h1_b;
#line 29 "tests/golden/run/adversarial-str-copy-out.hero"
    t10 = hero_str_len(t9);
#line 29 "tests/golden/run/adversarial-str-copy-out.hero"
    hero_print_int(t10);
#line 29 "tests/golden/run/adversarial-str-copy-out.hero"
    hero_print_end();
#line 29 "tests/golden/run/adversarial-str-copy-out.hero"
    t13 = h0_a;
#line 267 "adversarialstrcopyout.c"
    hero_str_decref(t13);
#line 29 "tests/golden/run/adversarial-str-copy-out.hero"
    t14 = h1_b;
#line 271 "adversarialstrcopyout.c"
    hero_str_decref(t14);
    return;
}
void h_adversarialstrcopyout_0opt0_retain(const h_adversarialstrcopyout_0opt0 *v) {
    if (v->tag == INT64_C(0)) {
        hero_str_incref(v->as.ok);
    } else {
        hero_failure_retain(&v->as.err);
    }
}

void h_adversarialstrcopyout_0opt0_release(h_adversarialstrcopyout_0opt0 *v) {
    if (v->tag == INT64_C(0)) {
        hero_str_decref(v->as.ok);
    } else {
        hero_failure_release(&v->as.err);
    }
}

bool h_adversarialstrcopyout_0opt0_eq(const h_adversarialstrcopyout_0opt0 *a, const h_adversarialstrcopyout_0opt0 *b) {
    if (a->tag != b->tag) return false;
    if (a->tag != INT64_C(0)) return hero_failure_eq(&a->as.err, &b->as.err);
    return (&hero_desc_str)->eq(&a->as.ok, &b->as.ok);
}

uint64_t h_adversarialstrcopyout_0opt0_hash(const void *elem) {
    const h_adversarialstrcopyout_0opt0 *v = elem;
    uint64_t h = (UINT64_C(0xcbf29ce484222325) ^ (uint64_t)v->tag)
        * UINT64_C(0x100000001b3);
    if (v->tag != INT64_C(0)) {
        return (h ^ hero_failure_hash(&v->as.err)) * UINT64_C(0x100000001b3);
    }
    return (h ^ (&hero_desc_str)->hash(&v->as.ok)) * UINT64_C(0x100000001b3);
}

void h_adversarialstrcopyout_0opt1_retain(const h_adversarialstrcopyout_0opt1 *v) {
    if (v->tag == INT64_C(0)) {
        return;
    } else {
        hero_failure_retain(&v->as.err);
    }
}

void h_adversarialstrcopyout_0opt1_release(h_adversarialstrcopyout_0opt1 *v) {
    if (v->tag == INT64_C(0)) {
        return;
    } else {
        hero_failure_release(&v->as.err);
    }
}

bool h_adversarialstrcopyout_0opt1_eq(const h_adversarialstrcopyout_0opt1 *a, const h_adversarialstrcopyout_0opt1 *b) {
    if (a->tag != b->tag) return false;
    if (a->tag != INT64_C(0)) return hero_failure_eq(&a->as.err, &b->as.err);
    return true;
}

uint64_t h_adversarialstrcopyout_0opt1_hash(const void *elem) {
    const h_adversarialstrcopyout_0opt1 *v = elem;
    uint64_t h = (UINT64_C(0xcbf29ce484222325) ^ (uint64_t)v->tag)
        * UINT64_C(0x100000001b3);
    if (v->tag != INT64_C(0)) {
        return (h ^ hero_failure_hash(&v->as.err)) * UINT64_C(0x100000001b3);
    }
    return h;
}


int main(int argc, char **argv) {
    hero_args_set(argc, argv);
    h_adversarialstrcopyout_main();
    hero_runtime_check_leaks();
    return 0;
}
