...
#line 53 "abortstrslice.c"

HERO_STR_STATIC(hero_str_0, "abcde");

typedef struct h_abortstrslice_0opt0 {
    int64_t tag;
    union {
        HeroStr ok;
        HeroFailure err;
    } as;
} h_abortstrslice_0opt0;

typedef struct h_abortstrslice_0opt1 {
    int64_t tag;
    union {
        HeroFailure err;
    } as;
} h_abortstrslice_0opt1;

void h_abortstrslice_0opt0_retain(const h_abortstrslice_0opt0 *v);
void h_abortstrslice_0opt0_release(h_abortstrslice_0opt0 *v);
bool h_abortstrslice_0opt0_eq(const h_abortstrslice_0opt0 *a, const h_abortstrslice_0opt0 *b);
uint64_t h_abortstrslice_0opt0_hash(const void *elem);
void h_abortstrslice_0opt1_retain(const h_abortstrslice_0opt1 *v);
void h_abortstrslice_0opt1_release(h_abortstrslice_0opt1 *v);
bool h_abortstrslice_0opt1_eq(const h_abortstrslice_0opt1 *a, const h_abortstrslice_0opt1 *b);
uint64_t h_abortstrslice_0opt1_hash(const void *elem);

void h_abortstrslice_main(void);

#line 6 "tests/golden/run/abort-str-slice.hero"
void h_abortstrslice_main(void) {
#line 85 "abortstrslice.c"
    HeroStr h0_s = {0};
    HeroStr h1_own1 = {0};
    HeroStr h2_own2 = {0};
    HeroStr t1 = {0};
...
#line 7 "tests/golden/run/abort-str-slice.hero"
    t10 = h0_s;
#line 110 "abortstrslice.c"
    hero_str_incref(t1);
#line 7 "tests/golden/run/abort-str-slice.hero"
    h0_s = t1;
#line 114 "abortstrslice.c"
    hero_str_decref(t10);
...
uint64_t h_abortstrslice_0opt0_hash(const void *elem) {
    const h_abortstrslice_0opt0 *v = elem;
...
uint64_t h_abortstrslice_0opt1_hash(const void *elem) {
    const h_abortstrslice_0opt1 *v = elem;
    uint64_t h = (UINT64_C(0xcbf29ce484222325) ^ (uint64_t)v->tag)
        * UINT64_C(0x100000001b3);
    if (v->tag != INT64_C(0)) {
        return (h ^ hero_failure_hash(&v->as.err)) * UINT64_C(0x100000001b3);
