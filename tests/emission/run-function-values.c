/* Generated by heroes 0.0.1 from tests/golden/run/function-values.hero. Do not edit. */
#include "heroes_runtime.h"
#include <math.h>
#include <hero_os.h>

_Static_assert(HERO_RUNTIME_ABI == 15, "heroes_runtime.h is from another compiler");

#define HERO_RET_INT(c) _Generic((c), signed char:1, short:1, int:1, long:1, long long:1, unsigned char:1, unsigned short:1, unsigned int:1, default:0)
#define HERO_C_INTEGER(c) _Generic((c), _Bool:1, char:1, signed char:1, short:1, int:1, long:1, long long:1, unsigned char:1, unsigned short:1, unsigned int:1, unsigned long:1, unsigned long long:1, default:0)
#define HERO_C_UNSIGNED(c) _Generic((c), unsigned char:1, unsigned short:1, unsigned int:1, unsigned long:1, unsigned long long:1, _Bool:1, char:((char)-1 > 0), signed char:0, short:0, int:0, long:0, long long:0, default:0)
#define HERO_RET_I8(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 1)
#define HERO_RET_I16(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 2)
#define HERO_RET_I32(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 4)
#define HERO_RET_U8(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 1)
#define HERO_RET_U16(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 2)
#define HERO_RET_U32(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 4)
#define HERO_RET_U64(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 8)
#define HERO_RET_F32(c) _Generic((c), float:1, default:0)
#define HERO_RET_F64(c) _Generic((c), float:1, double:1, long double:1, default:0)
#define HERO_RET_BOOL(c) _Generic((c), _Bool:1, default:0)
#define HERO_RET_STR(c) _Generic((c), HeroStr:1, default:0)
#define HERO_RET_UNIT(c) __builtin_types_compatible_p(__typeof__(c), void)
#define HERO_RET_PTR(c) (__builtin_classify_type(c) == 5)
_Static_assert(HERO_RET_INT(HERO_OS_OK), "heroes-ffi-return HERO_OS_OK i64");
_Static_assert(__builtin_constant_p(HERO_OS_OK), "heroes-ffi-const HERO_OS_OK");
_Static_assert(HERO_RET_INT(HERO_OS_NOT_FOUND), "heroes-ffi-return HERO_OS_NOT_FOUND i64");
_Static_assert(__builtin_constant_p(HERO_OS_NOT_FOUND), "heroes-ffi-const HERO_OS_NOT_FOUND");
_Static_assert(HERO_RET_STR(hero_file_read(0, (int64_t *)0)), "heroes-ffi-return hero_file_read str");
_Static_assert(HERO_RET_INT(hero_file_write(0, (HeroStr){0})), "heroes-ffi-return hero_file_write i64");
_Static_assert(HERO_RET_INT(hero_args_count()), "heroes-ffi-return hero_args_count i64");
_Static_assert(HERO_RET_STR(hero_args_at((int64_t)0)), "heroes-ffi-return hero_args_at str");
_Static_assert(HERO_RET_UNIT(hero_exit((int64_t)0)), "heroes-ffi-return hero_exit ()");

#line 111 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_file_read(const char * a0, int64_t * a1) { (void)hero_file_read(a0, a1); }
#line 112 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_file_write(const char * a0, HeroStr a1) { (void)hero_file_write(a0, a1); }
#line 114 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_args_at(int64_t a0) { (void)hero_args_at(a0); }
#line 115 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_exit(int64_t a0) { (void)hero_exit(a0); }
#line 43 "functionvalues.c"

HERO_STR_STATIC(hero_str_0, "tick");
HERO_STR_STATIC(hero_str_1, " ");

typedef int64_t (*h_functionvalues_0fn0)(int64_t, int64_t);
typedef void (*h_functionvalues_0fn1)(void);
typedef struct h_functionvalues_0opt0 {
    int64_t tag;
    union {
        HeroStr ok;
        HeroFailure err;
    } as;
} h_functionvalues_0opt0;

typedef struct h_functionvalues_0opt1 {
    int64_t tag;
    union {
        HeroFailure err;
    } as;
} h_functionvalues_0opt1;

void h_functionvalues_0opt0_retain(const h_functionvalues_0opt0 *v);
void h_functionvalues_0opt0_release(h_functionvalues_0opt0 *v);
bool h_functionvalues_0opt0_eq(const h_functionvalues_0opt0 *a, const h_functionvalues_0opt0 *b);
uint64_t h_functionvalues_0opt0_hash(const void *elem);
void h_functionvalues_0opt1_retain(const h_functionvalues_0opt1 *v);
void h_functionvalues_0opt1_release(h_functionvalues_0opt1 *v);
bool h_functionvalues_0opt1_eq(const h_functionvalues_0opt1 *a, const h_functionvalues_0opt1 *b);
uint64_t h_functionvalues_0opt1_hash(const void *elem);

int64_t h_functionvalues_add(int64_t h0_a, int64_t h1_b);
int64_t h_functionvalues_mul(int64_t h0_a, int64_t h1_b);
int64_t h_functionvalues_apply(h_functionvalues_0fn0 h0_f, int64_t h1_x, int64_t h2_y);
void h_functionvalues_tick(void);
void h_functionvalues_twice(h_functionvalues_0fn1 h0_f);
void h_functionvalues_main(void);

#line 27 "tests/golden/run/function-values.hero"
int64_t h_functionvalues_add(int64_t h0_a, int64_t h1_b) {
#line 83 "functionvalues.c"
    int64_t t1;
    int64_t t2;
    int64_t t3;
    goto bb0;
bb0:
#line 28 "tests/golden/run/function-values.hero"
    t1 = h0_a;
#line 28 "tests/golden/run/function-values.hero"
    t2 = h1_b;
#line 28 "tests/golden/run/function-values.hero"
    if (__builtin_add_overflow(t1, t2, &t3)) hero_panic_overflow();
#line 28 "tests/golden/run/function-values.hero"
    return t3;
#line 97 "functionvalues.c"
}

#line 30 "tests/golden/run/function-values.hero"
int64_t h_functionvalues_mul(int64_t h0_a, int64_t h1_b) {
#line 102 "functionvalues.c"
    int64_t t1;
    int64_t t2;
    int64_t t3;
    goto bb0;
bb0:
#line 31 "tests/golden/run/function-values.hero"
    t1 = h0_a;
#line 31 "tests/golden/run/function-values.hero"
    t2 = h1_b;
#line 31 "tests/golden/run/function-values.hero"
    if (__builtin_mul_overflow(t1, t2, &t3)) hero_panic_overflow();
#line 31 "tests/golden/run/function-values.hero"
    return t3;
#line 116 "functionvalues.c"
}

#line 33 "tests/golden/run/function-values.hero"
int64_t h_functionvalues_apply(h_functionvalues_0fn0 h0_f, int64_t h1_x, int64_t h2_y) {
#line 121 "functionvalues.c"
    h_functionvalues_0fn0 t1;
    int64_t t2;
    int64_t t3;
    int64_t t4;
    goto bb0;
bb0:
#line 34 "tests/golden/run/function-values.hero"
    t1 = h0_f;
#line 34 "tests/golden/run/function-values.hero"
    t2 = h1_x;
#line 34 "tests/golden/run/function-values.hero"
    t3 = h2_y;
#line 34 "tests/golden/run/function-values.hero"
    t4 = t1(t2, t3);
#line 34 "tests/golden/run/function-values.hero"
    return t4;
#line 138 "functionvalues.c"
}

#line 36 "tests/golden/run/function-values.hero"
void h_functionvalues_tick(void) {
#line 143 "functionvalues.c"
    HeroStr t1 = {0};
    goto bb0;
bb0:
#line 37 "tests/golden/run/function-values.hero"
    t1 = HERO_STR_LIT(hero_str_0);
#line 37 "tests/golden/run/function-values.hero"
    hero_print_str(t1);
#line 37 "tests/golden/run/function-values.hero"
    hero_print_end();
#line 37 "tests/golden/run/function-values.hero"
    return;
#line 155 "functionvalues.c"
}

#line 39 "tests/golden/run/function-values.hero"
void h_functionvalues_twice(h_functionvalues_0fn1 h0_f) {
#line 160 "functionvalues.c"
    h_functionvalues_0fn1 t1;
    h_functionvalues_0fn1 t2;
    goto bb0;
bb0:
#line 40 "tests/golden/run/function-values.hero"
    t1 = h0_f;
#line 40 "tests/golden/run/function-values.hero"
    t1();
#line 41 "tests/golden/run/function-values.hero"
    t2 = h0_f;
#line 41 "tests/golden/run/function-values.hero"
    t2();
#line 41 "tests/golden/run/function-values.hero"
    return;
#line 175 "functionvalues.c"
}

#line 43 "tests/golden/run/function-values.hero"
void h_functionvalues_main(void) {
#line 180 "functionvalues.c"
    h_functionvalues_0fn0 h0_g;
    HeroArrayHeader * h1_ops = {0};
    h_functionvalues_0fn0 h2_h;
    HeroArrayHeader * h3_own3 = {0};
    h_functionvalues_0fn0 t1;
    h_functionvalues_0fn0 t2;
    int64_t t3;
    int64_t t4;
    int64_t t5;
    HeroStr t6 = {0};
    h_functionvalues_0fn0 t7;
    int64_t t8;
    int64_t t9;
    int64_t t10;
    HeroStr t11 = {0};
    h_functionvalues_0fn0 t12;
    int64_t t13;
    int64_t t14;
    int64_t t15;
    h_functionvalues_0fn1 t16;
    h_functionvalues_0fn0 t17;
    h_functionvalues_0fn0 t18;
    HeroArrayHeader * t19 = {0};
    HeroArrayHeader * t20 = {0};
    int64_t t21;
    h_functionvalues_0fn0 t22;
    int64_t t23;
    int64_t t24;
    int64_t t25;
    HeroStr t26 = {0};
    HeroArrayHeader * t27 = {0};
    int64_t t28;
    h_functionvalues_0fn0 t29;
    int64_t t30;
    int64_t t31;
    int64_t t32;
    HeroStr t33 = {0};
    HeroArrayHeader * t34 = {0};
    int64_t t35;
    h_functionvalues_0fn0 t36;
    h_functionvalues_0fn0 t37;
    h_functionvalues_0fn0 t38;
    bool t39;
    HeroStr t40 = {0};
    h_functionvalues_0fn0 t41;
    h_functionvalues_0fn0 t42;
    bool t43;
    HeroArrayHeader * t44 = {0};
    HeroArrayHeader * t45 = {0};
    HeroArrayHeader * t46 = {0};
    HeroArrayHeader * t47 = {0};
    goto bb0;
bb0:
#line 44 "tests/golden/run/function-values.hero"
    t1 = h_functionvalues_add;
#line 44 "tests/golden/run/function-values.hero"
    h0_g = t1;
#line 45 "tests/golden/run/function-values.hero"
    t2 = h0_g;
#line 45 "tests/golden/run/function-values.hero"
    t3 = INT64_C(2);
#line 45 "tests/golden/run/function-values.hero"
    t4 = INT64_C(3);
#line 45 "tests/golden/run/function-values.hero"
    t5 = t2(t3, t4);
#line 45 "tests/golden/run/function-values.hero"
    t6 = HERO_STR_LIT(hero_str_1);
#line 45 "tests/golden/run/function-values.hero"
    t7 = h_functionvalues_add;
#line 45 "tests/golden/run/function-values.hero"
    t8 = INT64_C(2);
#line 45 "tests/golden/run/function-values.hero"
    t9 = INT64_C(3);
#line 45 "tests/golden/run/function-values.hero"
    t10 = h_functionvalues_apply(t7, t8, t9);
#line 45 "tests/golden/run/function-values.hero"
    t11 = HERO_STR_LIT(hero_str_1);
#line 45 "tests/golden/run/function-values.hero"
    t12 = h_functionvalues_mul;
#line 45 "tests/golden/run/function-values.hero"
    t13 = INT64_C(2);
#line 45 "tests/golden/run/function-values.hero"
    t14 = INT64_C(3);
#line 45 "tests/golden/run/function-values.hero"
    t15 = h_functionvalues_apply(t12, t13, t14);
#line 45 "tests/golden/run/function-values.hero"
    hero_print_int(t5);
#line 45 "tests/golden/run/function-values.hero"
    hero_print_str(t6);
#line 45 "tests/golden/run/function-values.hero"
    hero_print_int(t10);
#line 45 "tests/golden/run/function-values.hero"
    hero_print_str(t11);
#line 45 "tests/golden/run/function-values.hero"
    hero_print_int(t15);
#line 45 "tests/golden/run/function-values.hero"
    hero_print_end();
#line 47 "tests/golden/run/function-values.hero"
    t16 = h_functionvalues_tick;
#line 47 "tests/golden/run/function-values.hero"
    h_functionvalues_twice(t16);
#line 51 "tests/golden/run/function-values.hero"
    t17 = h_functionvalues_add;
#line 51 "tests/golden/run/function-values.hero"
    t18 = h_functionvalues_mul;
#line 51 "tests/golden/run/function-values.hero"
    t19 = hero_array_new(&hero_desc_func, 2);
#line 51 "tests/golden/run/function-values.hero"
    { HeroArrayHeader *grown = hero_array_push(t19, &t17);
#line 51 "tests/golden/run/function-values.hero"
      hero_array_decref(t19); t19 = grown; }
#line 51 "tests/golden/run/function-values.hero"
    { HeroArrayHeader *grown = hero_array_push(t19, &t18);
#line 51 "tests/golden/run/function-values.hero"
      hero_array_decref(t19); t19 = grown; }
#line 51 "tests/golden/run/function-values.hero"
    t44 = h3_own3;
#line 51 "tests/golden/run/function-values.hero"
    h3_own3 = t19;
#line 300 "functionvalues.c"
    hero_array_decref(t44);
#line 51 "tests/golden/run/function-values.hero"
    t45 = h1_ops;
#line 304 "functionvalues.c"
    hero_array_incref(t19);
#line 51 "tests/golden/run/function-values.hero"
    h1_ops = t19;
#line 308 "functionvalues.c"
    hero_array_decref(t45);
#line 52 "tests/golden/run/function-values.hero"
    t20 = h1_ops;
#line 52 "tests/golden/run/function-values.hero"
    t21 = INT64_C(0);
#line 52 "tests/golden/run/function-values.hero"
    t22 = *(h_functionvalues_0fn0 const *)hero_array_at(t20, t21);
#line 52 "tests/golden/run/function-values.hero"
    t23 = INT64_C(4);
#line 52 "tests/golden/run/function-values.hero"
    t24 = INT64_C(5);
#line 52 "tests/golden/run/function-values.hero"
    t25 = t22(t23, t24);
#line 52 "tests/golden/run/function-values.hero"
    t26 = HERO_STR_LIT(hero_str_1);
#line 52 "tests/golden/run/function-values.hero"
    t27 = h1_ops;
#line 52 "tests/golden/run/function-values.hero"
    t28 = INT64_C(1);
#line 52 "tests/golden/run/function-values.hero"
    t29 = *(h_functionvalues_0fn0 const *)hero_array_at(t27, t28);
#line 52 "tests/golden/run/function-values.hero"
    t30 = INT64_C(4);
#line 52 "tests/golden/run/function-values.hero"
    t31 = INT64_C(5);
#line 52 "tests/golden/run/function-values.hero"
    t32 = t29(t30, t31);
#line 52 "tests/golden/run/function-values.hero"
    t33 = HERO_STR_LIT(hero_str_1);
#line 52 "tests/golden/run/function-values.hero"
    t34 = h1_ops;
#line 52 "tests/golden/run/function-values.hero"
    t35 = hero_array_len(t34);
#line 52 "tests/golden/run/function-values.hero"
    hero_print_int(t25);
#line 52 "tests/golden/run/function-values.hero"
    hero_print_str(t26);
#line 52 "tests/golden/run/function-values.hero"
    hero_print_int(t32);
#line 52 "tests/golden/run/function-values.hero"
    hero_print_str(t33);
#line 52 "tests/golden/run/function-values.hero"
    hero_print_int(t35);
#line 52 "tests/golden/run/function-values.hero"
    hero_print_end();
#line 55 "tests/golden/run/function-values.hero"
    t36 = h_functionvalues_add;
#line 55 "tests/golden/run/function-values.hero"
    h2_h = t36;
#line 56 "tests/golden/run/function-values.hero"
    t37 = h0_g;
#line 56 "tests/golden/run/function-values.hero"
    t38 = h2_h;
#line 56 "tests/golden/run/function-values.hero"
    t39 = t37 == t38;
#line 56 "tests/golden/run/function-values.hero"
    t40 = HERO_STR_LIT(hero_str_1);
#line 56 "tests/golden/run/function-values.hero"
    t41 = h0_g;
#line 56 "tests/golden/run/function-values.hero"
    t42 = h_functionvalues_mul;
#line 56 "tests/golden/run/function-values.hero"
    t43 = t41 == t42;
#line 56 "tests/golden/run/function-values.hero"
    hero_print_bool(t39);
#line 56 "tests/golden/run/function-values.hero"
    hero_print_str(t40);
#line 56 "tests/golden/run/function-values.hero"
    hero_print_bool(t43);
#line 56 "tests/golden/run/function-values.hero"
    hero_print_end();
#line 56 "tests/golden/run/function-values.hero"
    t46 = h1_ops;
#line 382 "functionvalues.c"
    hero_array_decref(t46);
#line 56 "tests/golden/run/function-values.hero"
    t47 = h3_own3;
#line 386 "functionvalues.c"
    hero_array_decref(t47);
    return;
}
void h_functionvalues_0opt0_retain(const h_functionvalues_0opt0 *v) {
    if (v->tag == INT64_C(0)) {
        hero_str_incref(v->as.ok);
    } else {
        hero_failure_retain(&v->as.err);
    }
}

void h_functionvalues_0opt0_release(h_functionvalues_0opt0 *v) {
    if (v->tag == INT64_C(0)) {
        hero_str_decref(v->as.ok);
    } else {
        hero_failure_release(&v->as.err);
    }
}

bool h_functionvalues_0opt0_eq(const h_functionvalues_0opt0 *a, const h_functionvalues_0opt0 *b) {
    if (a->tag != b->tag) return false;
    if (a->tag != INT64_C(0)) return hero_failure_eq(&a->as.err, &b->as.err);
    return (&hero_desc_str)->eq(&a->as.ok, &b->as.ok);
}

uint64_t h_functionvalues_0opt0_hash(const void *elem) {
    const h_functionvalues_0opt0 *v = elem;
    uint64_t h = (UINT64_C(0xcbf29ce484222325) ^ (uint64_t)v->tag)
        * UINT64_C(0x100000001b3);
    if (v->tag != INT64_C(0)) {
        return (h ^ hero_failure_hash(&v->as.err)) * UINT64_C(0x100000001b3);
    }
    return (h ^ (&hero_desc_str)->hash(&v->as.ok)) * UINT64_C(0x100000001b3);
}

void h_functionvalues_0opt1_retain(const h_functionvalues_0opt1 *v) {
    if (v->tag == INT64_C(0)) {
        return;
    } else {
        hero_failure_retain(&v->as.err);
    }
}

void h_functionvalues_0opt1_release(h_functionvalues_0opt1 *v) {
    if (v->tag == INT64_C(0)) {
        return;
    } else {
        hero_failure_release(&v->as.err);
    }
}

bool h_functionvalues_0opt1_eq(const h_functionvalues_0opt1 *a, const h_functionvalues_0opt1 *b) {
    if (a->tag != b->tag) return false;
    if (a->tag != INT64_C(0)) return hero_failure_eq(&a->as.err, &b->as.err);
    return true;
}

uint64_t h_functionvalues_0opt1_hash(const void *elem) {
    const h_functionvalues_0opt1 *v = elem;
    uint64_t h = (UINT64_C(0xcbf29ce484222325) ^ (uint64_t)v->tag)
        * UINT64_C(0x100000001b3);
    if (v->tag != INT64_C(0)) {
        return (h ^ hero_failure_hash(&v->as.err)) * UINT64_C(0x100000001b3);
    }
    return h;
}


int main(int argc, char **argv) {
    hero_args_set(argc, argv);
    h_functionvalues_main();
    hero_runtime_check_leaks();
    return 0;
}
