/* Generated by heroes 0.1.0 from tests/golden/ir/owned-cell.hero. Do not edit. */
#include "heroes_runtime.h"
#include <math.h>
#include <hero_os.h>
#include <stdlib.h>
#include <err.h>
#include <heroes_runtime.h>

_Static_assert(HERO_RUNTIME_ABI == 21, "heroes_runtime.h is from another compiler");
#define HERO_TU_LOCAL
#define HERO_TU_QUIET

#define HERO_RET_INT(c) _Generic((c), signed char:1, short:1, int:1, long:1, long long:1, unsigned char:1, unsigned short:1, unsigned int:1, default:0)
#define HERO_C_INTEGER(c) _Generic((c), _Bool:1, char:1, signed char:1, short:1, int:1, long:1, long long:1, unsigned char:1, unsigned short:1, unsigned int:1, unsigned long:1, unsigned long long:1, default:0)
#define HERO_C_UNSIGNED(c) _Generic((c), unsigned char:1, unsigned short:1, unsigned int:1, unsigned long:1, unsigned long long:1, _Bool:1, char:((char)-1 > 0), signed char:0, short:0, int:0, long:0, long long:0, default:0)
#define HERO_RET_I8(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 1)
#define HERO_RET_I16(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 2)
#define HERO_RET_I32(c) (HERO_C_INTEGER(c) && !HERO_C_UNSIGNED(c) && sizeof(c) == 4)
#define HERO_RET_U8(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 1)
#define HERO_RET_U16(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 2)
#define HERO_RET_U32(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 4)
#define HERO_RET_U64(c) (HERO_C_INTEGER(c) && HERO_C_UNSIGNED(c) && sizeof(c) == 8)
#define HERO_RET_F32(c) _Generic((c), float:1, default:0)
#define HERO_RET_F64(c) _Generic((c), float:1, double:1, long double:1, default:0)
#define HERO_RET_BOOL(c) _Generic((c), _Bool:1, default:0)
#define HERO_RET_STR(c) _Generic((c), HeroStr:1, default:0)
#define HERO_RET_UNIT(c) __builtin_types_compatible_p(__typeof__(c), void)
#define HERO_RET_PTR(c) (__builtin_classify_type(c) == 5)
#define HERO_RET_CSTR(c) _Generic((c), char *:1, const char *:1, signed char *:1, const signed char *:1, unsigned char *:1, const unsigned char *:1, default:0)
_Static_assert(HERO_RET_UNIT(free((void *)0)), "heroes-ffi-return free ()");
_Static_assert(HERO_RET_I32(describe((int32_t)0, 0)), "heroes-ffi-return describe i32");
_Static_assert(HERO_RET_INT(HERO_OS_OK), "heroes-ffi-return HERO_OS_OK i64");
_Static_assert(__builtin_constant_p(HERO_OS_OK), "heroes-ffi-const HERO_OS_OK");
_Static_assert(HERO_RET_INT(HERO_OS_NOT_FOUND), "heroes-ffi-return HERO_OS_NOT_FOUND i64");
_Static_assert(__builtin_constant_p(HERO_OS_NOT_FOUND), "heroes-ffi-const HERO_OS_NOT_FOUND");
_Static_assert(HERO_RET_INT(HERO_OS_NOT_TEXT), "heroes-ffi-return HERO_OS_NOT_TEXT i64");
_Static_assert(__builtin_constant_p(HERO_OS_NOT_TEXT), "heroes-ffi-const HERO_OS_NOT_TEXT");
_Static_assert(HERO_RET_STR(hero_file_read(0, 0)), "heroes-ffi-return hero_file_read str");
_Static_assert(HERO_RET_INT(hero_file_write(0, (HeroStr){0})), "heroes-ffi-return hero_file_write i64");
_Static_assert(HERO_RET_INT(hero_args_count()), "heroes-ffi-return hero_args_count i64");
_Static_assert(HERO_RET_STR(hero_args_at((int64_t)0)), "heroes-ffi-return hero_args_at str");
_Static_assert(HERO_RET_CSTR(hero_args_raw((int64_t)0)), "heroes-ffi-return hero_args_raw cstr");
_Static_assert(HERO_RET_UNIT(hero_exit((int64_t)0)), "heroes-ffi-return hero_exit ()");
_Static_assert(HERO_RET_INT(HERO_STR_OK), "heroes-ffi-return HERO_STR_OK i64");
_Static_assert(__builtin_constant_p(HERO_STR_OK), "heroes-ffi-const HERO_STR_OK");
_Static_assert(HERO_RET_STR(hero_str_try_from_cstr(0, 0)), "heroes-ffi-return hero_str_try_from_cstr str");

HERO_STR_STATIC(hero_str_edaa230, "none");
HERO_STR_STATIC(hero_str_37c08f66, "nothing yet");
HERO_STR_STATIC(hero_str_6ad4aa47, "no reason given");
HERO_STR_STATIC(hero_str_4a05a740, "null_cstr");
HERO_STR_STATIC(hero_str_5263489, "a null `cstr` holds no text");
HERO_STR_STATIC(hero_str_34624695, "not_text");
HERO_STR_STATIC(hero_str_4bb64adb, "the bytes behind this `cstr` are not UTF-8");

#line 57 "ownedcell.c"
typedef struct h_0opt_f87774a {
    int64_t tag;
    union {
        HeroStr ok;
        HeroFailure err;
    } as;
} h_0opt_f87774a;

typedef struct h_0opt_a8ea2 {
    int64_t tag;
    union {
        HeroFailure err;
    } as;
} h_0opt_a8ea2;

#pragma clang diagnostic push
#pragma clang diagnostic error "-Wdouble-promotion"
#pragma clang diagnostic error "-Wimplicit-float-conversion"
#pragma clang diagnostic error "-Wfloat-conversion"
#pragma clang diagnostic error "-Wimplicit-int-conversion"
#line 41 "tests/golden/ir/owned-cell.hero"
__attribute__((unused)) static void hero_ffi_probe_h_ownedcell_free(char * a0) { (void)(free)(a0); }
#line 44 "tests/golden/ir/owned-cell.hero"
__attribute__((unused)) static void hero_ffi_probe_h_ownedcell_describe(int32_t a0, char * * a1) { (void)(describe)(a0, a1); }
#line 112 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_file_read(const char * a0, int64_t * a1) { (void)(hero_file_read)(a0, (void *)a1); }
#line 113 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_file_write(const char * a0, HeroStr a1) { (void)(hero_file_write)(a0, a1); }
#line 115 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_args_at(int64_t a0) { (void)(hero_args_at)(a0); }
#line 116 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_args_raw(int64_t a0) { (void)(hero_args_raw)(a0); }
#line 117 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_exit(int64_t a0) { (void)(hero_exit)(a0); }
#line 123 "<heroes library>"
__attribute__((unused)) static void hero_ffi_probe_h_library_hero_str_try_from_cstr(const char * a0, int64_t * a1) { (void)(hero_str_try_from_cstr)(a0, (void *)a1); }
#line 94 "ownedcell.c"
#pragma clang diagnostic pop

HERO_TU_LOCAL void h_0opt_f87774a_retain(const h_0opt_f87774a *v);
HERO_TU_LOCAL void h_0opt_f87774a_release(h_0opt_f87774a *v);
HERO_TU_LOCAL bool h_0opt_f87774a_eq(const h_0opt_f87774a *a, const h_0opt_f87774a *b);
HERO_TU_LOCAL uint64_t h_0opt_f87774a_hash(const void *elem);
HERO_TU_LOCAL void h_0opt_a8ea2_retain(const h_0opt_a8ea2 *v);
HERO_TU_LOCAL void h_0opt_a8ea2_release(h_0opt_a8ea2 *v);
HERO_TU_LOCAL bool h_0opt_a8ea2_eq(const h_0opt_a8ea2 *a, const h_0opt_a8ea2 *b);
HERO_TU_LOCAL uint64_t h_0opt_a8ea2_hash(const void *elem);

HeroStr h_ownedcell_reason(int32_t h0_code);
int64_t h_library_HERO_STR_OK(void);
h_0opt_f87774a h_library_validated(const char * h0_c);

#line 46 "tests/golden/ir/owned-cell.hero"
HeroStr h_ownedcell_reason(int32_t h0_code) {
#line 112 "ownedcell.c"
    h_0opt_f87774a h1_message = {0};
    const char * h2_cell0;
    const char * h3_owned0;
    h_0opt_f87774a h4_owned1 = {0};
    h_0opt_f87774a h5_f0 = {0};
    HeroStr h6_r0 = {0};
    h_0opt_f87774a h7_own7 = {0};
    h_0opt_f87774a h8_own8 = {0};
    HeroStr t1 = {0};
    HeroStr t2 = {0};
    h_0opt_f87774a t3 = {0};
    int32_t t4;
    const char * t5;
    const char * t7;
    const char * t8;
    h_0opt_f87774a t9 = {0};
    const char * t10;
    const char * t11;
    bool t12;
    const char * t13;
    void * t14;
    h_0opt_f87774a t15 = {0};
    h_0opt_f87774a t16 = {0};
    h_0opt_f87774a t17 = {0};
    int64_t t18;
    int64_t t19;
    bool t20;
    h_0opt_f87774a t21 = {0};
    HeroStr t22 = {0};
    HeroStr t23 = {0};
    HeroStr t24 = {0};
    h_0opt_f87774a t25 = {0};
    h_0opt_f87774a t26 = {0};
    h_0opt_f87774a t27 = {0};
    h_0opt_f87774a t28 = {0};
    h_0opt_f87774a t29 = {0};
    h_0opt_f87774a t30 = {0};
    HeroStr t31 = {0};
    HeroStr t32 = {0};
    goto bb0;
bb0:
#line 47 "tests/golden/ir/owned-cell.hero"
    t1 = HERO_STR_LIT(hero_str_edaa230);
#line 47 "tests/golden/ir/owned-cell.hero"
    t2 = HERO_STR_LIT(hero_str_37c08f66);
#line 158 "ownedcell.c"
    hero_str_incref(t1);
#line 47 "tests/golden/ir/owned-cell.hero"
#line 161 "ownedcell.c"
    hero_str_incref(t2);
#line 47 "tests/golden/ir/owned-cell.hero"
    t3 = (h_0opt_f87774a){.tag = INT64_C(1), .as.err = {.code = t1, .msg = t2}};
#line 47 "tests/golden/ir/owned-cell.hero"
    t25 = h7_own7;
#line 47 "tests/golden/ir/owned-cell.hero"
    h7_own7 = t3;
#line 169 "ownedcell.c"
    h_0opt_f87774a_release(&t25);
#line 47 "tests/golden/ir/owned-cell.hero"
    t26 = h1_message;
#line 173 "ownedcell.c"
    h_0opt_f87774a_retain(&t3);
#line 47 "tests/golden/ir/owned-cell.hero"
    h1_message = t3;
#line 177 "ownedcell.c"
    h_0opt_f87774a_release(&t26);
#line 48 "tests/golden/ir/owned-cell.hero"
    t4 = h0_code;
#line 48 "tests/golden/ir/owned-cell.hero"
    t5 = ((void *)0);
#line 48 "tests/golden/ir/owned-cell.hero"
    h2_cell0 = t5;
#line 48 "tests/golden/ir/owned-cell.hero"
    (void)describe(t4, (char **)&h2_cell0);
#line 48 "tests/golden/ir/owned-cell.hero"
    t7 = h2_cell0;
#line 48 "tests/golden/ir/owned-cell.hero"
    h3_owned0 = t7;
#line 48 "tests/golden/ir/owned-cell.hero"
    t8 = h3_owned0;
#line 48 "tests/golden/ir/owned-cell.hero"
    t9 = h_library_validated(t8);
#line 48 "tests/golden/ir/owned-cell.hero"
    t27 = h8_own8;
#line 48 "tests/golden/ir/owned-cell.hero"
    h8_own8 = t9;
#line 199 "ownedcell.c"
    h_0opt_f87774a_release(&t27);
#line 48 "tests/golden/ir/owned-cell.hero"
    t28 = h4_owned1;
#line 203 "ownedcell.c"
    h_0opt_f87774a_retain(&t9);
#line 48 "tests/golden/ir/owned-cell.hero"
    h4_owned1 = t9;
#line 207 "ownedcell.c"
    h_0opt_f87774a_release(&t28);
#line 48 "tests/golden/ir/owned-cell.hero"
    t10 = ((void *)0);
#line 48 "tests/golden/ir/owned-cell.hero"
    t11 = h3_owned0;
#line 48 "tests/golden/ir/owned-cell.hero"
    t12 = t11 != t10;
#line 48 "tests/golden/ir/owned-cell.hero"
    if (t12) goto bb1; else goto bb2;
#line 48 "tests/golden/ir/owned-cell.hero"
bb1:
#line 48 "tests/golden/ir/owned-cell.hero"
    t13 = h3_owned0;
#line 48 "tests/golden/ir/owned-cell.hero"
    t14 = (void *)(char *)t13;
#line 48 "tests/golden/ir/owned-cell.hero"
    (void)free(t14);
#line 48 "tests/golden/ir/owned-cell.hero"
    goto bb2;
#line 48 "tests/golden/ir/owned-cell.hero"
bb2:
#line 48 "tests/golden/ir/owned-cell.hero"
    t15 = h4_owned1;
#line 48 "tests/golden/ir/owned-cell.hero"
    t29 = h1_message;
#line 233 "ownedcell.c"
    h_0opt_f87774a_retain(&t15);
#line 48 "tests/golden/ir/owned-cell.hero"
    h1_message = t15;
#line 237 "ownedcell.c"
    h_0opt_f87774a_release(&t29);
#line 49 "tests/golden/ir/owned-cell.hero"
    t16 = h1_message;
#line 49 "tests/golden/ir/owned-cell.hero"
    t30 = h5_f0;
#line 243 "ownedcell.c"
    h_0opt_f87774a_retain(&t16);
#line 49 "tests/golden/ir/owned-cell.hero"
    h5_f0 = t16;
#line 247 "ownedcell.c"
    h_0opt_f87774a_release(&t30);
#line 49 "tests/golden/ir/owned-cell.hero"
    t17 = h5_f0;
#line 49 "tests/golden/ir/owned-cell.hero"
    t18 = t17.tag;
#line 49 "tests/golden/ir/owned-cell.hero"
    t19 = INT64_C(0);
#line 49 "tests/golden/ir/owned-cell.hero"
    t20 = t18 == t19;
#line 49 "tests/golden/ir/owned-cell.hero"
    if (t20) goto bb3; else goto bb4;
#line 49 "tests/golden/ir/owned-cell.hero"
bb3:
#line 49 "tests/golden/ir/owned-cell.hero"
    t21 = h5_f0;
#line 49 "tests/golden/ir/owned-cell.hero"
    t22 = t21.as.ok;
#line 49 "tests/golden/ir/owned-cell.hero"
    t31 = h6_r0;
#line 267 "ownedcell.c"
    hero_str_incref(t22);
#line 49 "tests/golden/ir/owned-cell.hero"
    h6_r0 = t22;
#line 271 "ownedcell.c"
    hero_str_decref(t31);
    goto bb5;
bb4:
#line 49 "tests/golden/ir/owned-cell.hero"
    t23 = HERO_STR_LIT(hero_str_6ad4aa47);
#line 49 "tests/golden/ir/owned-cell.hero"
    t32 = h6_r0;
#line 279 "ownedcell.c"
    hero_str_incref(t23);
#line 49 "tests/golden/ir/owned-cell.hero"
    h6_r0 = t23;
#line 283 "ownedcell.c"
    hero_str_decref(t32);
    goto bb5;
bb5:
#line 49 "tests/golden/ir/owned-cell.hero"
    t24 = h6_r0;
#line 289 "ownedcell.c"
    hero_str_incref(t24);
#line 49 "tests/golden/ir/owned-cell.hero"
#line 292 "ownedcell.c"
    h_0opt_f87774a_release(&h1_message);
#line 49 "tests/golden/ir/owned-cell.hero"
#line 295 "ownedcell.c"
    h_0opt_f87774a_release(&h4_owned1);
#line 49 "tests/golden/ir/owned-cell.hero"
#line 298 "ownedcell.c"
    h_0opt_f87774a_release(&h5_f0);
#line 49 "tests/golden/ir/owned-cell.hero"
#line 301 "ownedcell.c"
    hero_str_decref(h6_r0);
#line 49 "tests/golden/ir/owned-cell.hero"
#line 304 "ownedcell.c"
    h_0opt_f87774a_release(&h7_own7);
#line 49 "tests/golden/ir/owned-cell.hero"
#line 307 "ownedcell.c"
    h_0opt_f87774a_release(&h8_own8);
    return t24;
}

#line 122 "<heroes library>"
int64_t h_library_HERO_STR_OK(void) {
#line 314 "ownedcell.c"
    return HERO_STR_OK;
}

#line 152 "<heroes library>"
h_0opt_f87774a h_library_validated(const char * h0_c) {
#line 320 "ownedcell.c"
    int64_t h1_status;
    HeroStr h2_text = {0};
    HeroStr h3_own3 = {0};
    h_0opt_f87774a h4_own4 = {0};
    h_0opt_f87774a h5_own5 = {0};
    h_0opt_f87774a h6_own6 = {0};
    const char * t1;
    const char * t2;
    bool t3;
    HeroStr t4 = {0};
    HeroStr t5 = {0};
    h_0opt_f87774a t6 = {0};
    int64_t t7;
    const char * t8;
    HeroStr t9 = {0};
    int64_t t10;
    int64_t t11;
    bool t12;
    HeroStr t13 = {0};
    h_0opt_f87774a t14 = {0};
    HeroStr t15 = {0};
    HeroStr t16 = {0};
    h_0opt_f87774a t17 = {0};
    HeroStr t18 = {0};
    HeroStr t19 = {0};
    h_0opt_f87774a t20 = {0};
    h_0opt_f87774a t21 = {0};
    h_0opt_f87774a t22 = {0};
    goto bb0;
bb0:
#line 153 "<heroes library>"
    t1 = h0_c;
#line 153 "<heroes library>"
    t2 = ((void *)0);
#line 153 "<heroes library>"
    t3 = t1 == t2;
#line 153 "<heroes library>"
    if (t3) goto bb2; else goto bb3;
#line 153 "<heroes library>"
bb1:
#line 155 "<heroes library>"
    t7 = INT64_C(0);
#line 155 "<heroes library>"
    h1_status = t7;
#line 156 "<heroes library>"
    t8 = h0_c;
#line 156 "<heroes library>"
    t9 = hero_str_try_from_cstr(hero_cstr_nonnull(t8), (void *)&h1_status);
#line 156 "<heroes library>"
    t18 = h3_own3;
#line 156 "<heroes library>"
    h3_own3 = t9;
#line 373 "ownedcell.c"
    hero_str_decref(t18);
#line 156 "<heroes library>"
    t19 = h2_text;
#line 377 "ownedcell.c"
    hero_str_incref(t9);
#line 156 "<heroes library>"
    h2_text = t9;
#line 381 "ownedcell.c"
    hero_str_decref(t19);
#line 157 "<heroes library>"
    t10 = h1_status;
#line 157 "<heroes library>"
    t11 = h_library_HERO_STR_OK();
#line 157 "<heroes library>"
    t12 = t10 == t11;
#line 157 "<heroes library>"
    if (t12) goto bb5; else goto bb6;
#line 157 "<heroes library>"
bb2:
#line 154 "<heroes library>"
    t4 = HERO_STR_LIT(hero_str_4a05a740);
#line 154 "<heroes library>"
    t5 = HERO_STR_LIT(hero_str_5263489);
#line 397 "ownedcell.c"
    hero_str_incref(t4);
#line 154 "<heroes library>"
#line 400 "ownedcell.c"
    hero_str_incref(t5);
#line 154 "<heroes library>"
    t6 = (h_0opt_f87774a){.tag = INT64_C(1), .as.err = {.code = t4, .msg = t5}};
#line 154 "<heroes library>"
    t20 = h4_own4;
#line 154 "<heroes library>"
    h4_own4 = t6;
#line 408 "ownedcell.c"
    h_0opt_f87774a_release(&t20);
#line 154 "<heroes library>"
#line 411 "ownedcell.c"
    h_0opt_f87774a_retain(&t6);
#line 154 "<heroes library>"
#line 414 "ownedcell.c"
    hero_str_decref(h2_text);
#line 154 "<heroes library>"
#line 417 "ownedcell.c"
    hero_str_decref(h3_own3);
#line 154 "<heroes library>"
#line 420 "ownedcell.c"
    h_0opt_f87774a_release(&h4_own4);
#line 154 "<heroes library>"
#line 423 "ownedcell.c"
    h_0opt_f87774a_release(&h5_own5);
#line 154 "<heroes library>"
#line 426 "ownedcell.c"
    h_0opt_f87774a_release(&h6_own6);
    return t6;
bb3:
    goto bb1;
bb4:
#line 159 "<heroes library>"
    t15 = HERO_STR_LIT(hero_str_34624695);
#line 159 "<heroes library>"
    t16 = HERO_STR_LIT(hero_str_4bb64adb);
#line 436 "ownedcell.c"
    hero_str_incref(t15);
#line 159 "<heroes library>"
#line 439 "ownedcell.c"
    hero_str_incref(t16);
#line 159 "<heroes library>"
    t17 = (h_0opt_f87774a){.tag = INT64_C(1), .as.err = {.code = t15, .msg = t16}};
#line 159 "<heroes library>"
    t21 = h5_own5;
#line 159 "<heroes library>"
    h5_own5 = t17;
#line 447 "ownedcell.c"
    h_0opt_f87774a_release(&t21);
#line 159 "<heroes library>"
#line 450 "ownedcell.c"
    h_0opt_f87774a_retain(&t17);
#line 159 "<heroes library>"
#line 453 "ownedcell.c"
    hero_str_decref(h2_text);
#line 159 "<heroes library>"
#line 456 "ownedcell.c"
    hero_str_decref(h3_own3);
#line 159 "<heroes library>"
#line 459 "ownedcell.c"
    h_0opt_f87774a_release(&h4_own4);
#line 159 "<heroes library>"
#line 462 "ownedcell.c"
    h_0opt_f87774a_release(&h5_own5);
#line 159 "<heroes library>"
#line 465 "ownedcell.c"
    h_0opt_f87774a_release(&h6_own6);
    return t17;
bb5:
#line 158 "<heroes library>"
    t13 = h2_text;
#line 471 "ownedcell.c"
    hero_str_incref(t13);
#line 158 "<heroes library>"
    t14 = (h_0opt_f87774a){.tag = INT64_C(0), .as.ok = t13};
#line 158 "<heroes library>"
    t22 = h6_own6;
#line 158 "<heroes library>"
    h6_own6 = t14;
#line 479 "ownedcell.c"
    h_0opt_f87774a_release(&t22);
#line 158 "<heroes library>"
#line 482 "ownedcell.c"
    h_0opt_f87774a_retain(&t14);
#line 158 "<heroes library>"
#line 485 "ownedcell.c"
    hero_str_decref(h2_text);
#line 158 "<heroes library>"
#line 488 "ownedcell.c"
    hero_str_decref(h3_own3);
#line 158 "<heroes library>"
#line 491 "ownedcell.c"
    h_0opt_f87774a_release(&h4_own4);
#line 158 "<heroes library>"
#line 494 "ownedcell.c"
    h_0opt_f87774a_release(&h5_own5);
#line 158 "<heroes library>"
#line 497 "ownedcell.c"
    h_0opt_f87774a_release(&h6_own6);
    return t14;
bb6:
    goto bb4;
}
HERO_TU_LOCAL void h_0opt_f87774a_retain(const h_0opt_f87774a *v) {
    if (v->tag == INT64_C(0)) {
        hero_str_incref(v->as.ok);
    } else {
        hero_failure_retain(&v->as.err);
    }
}

HERO_TU_LOCAL void h_0opt_f87774a_release(h_0opt_f87774a *v) {
    if (v->tag == INT64_C(0)) {
        hero_str_decref(v->as.ok);
    } else {
        hero_failure_release(&v->as.err);
    }
}

HERO_TU_LOCAL bool h_0opt_f87774a_eq(const h_0opt_f87774a *a, const h_0opt_f87774a *b) {
    if (a->tag != b->tag) return false;
    if (a->tag != INT64_C(0)) return hero_failure_eq(&a->as.err, &b->as.err);
    return (&hero_desc_str)->eq(&a->as.ok, &b->as.ok);
}

HERO_TU_LOCAL uint64_t h_0opt_f87774a_hash(const void *elem) {
    const h_0opt_f87774a *v = elem;
    uint64_t h = (UINT64_C(0xcbf29ce484222325) ^ (uint64_t)v->tag)
        * UINT64_C(0x100000001b3);
    if (v->tag != INT64_C(0)) {
        return (h ^ hero_failure_hash(&v->as.err)) * UINT64_C(0x100000001b3);
    }
    return (h ^ (&hero_desc_str)->hash(&v->as.ok)) * UINT64_C(0x100000001b3);
}

HERO_TU_LOCAL void h_0opt_a8ea2_retain(const h_0opt_a8ea2 *v) {
    if (v->tag == INT64_C(0)) {
        return;
    } else {
        hero_failure_retain(&v->as.err);
    }
}

HERO_TU_LOCAL void h_0opt_a8ea2_release(h_0opt_a8ea2 *v) {
    if (v->tag == INT64_C(0)) {
        return;
    } else {
        hero_failure_release(&v->as.err);
    }
}

HERO_TU_LOCAL bool h_0opt_a8ea2_eq(const h_0opt_a8ea2 *a, const h_0opt_a8ea2 *b) {
    if (a->tag != b->tag) return false;
    if (a->tag != INT64_C(0)) return hero_failure_eq(&a->as.err, &b->as.err);
    return true;
}

HERO_TU_LOCAL uint64_t h_0opt_a8ea2_hash(const void *elem) {
    const h_0opt_a8ea2 *v = elem;
    uint64_t h = (UINT64_C(0xcbf29ce484222325) ^ (uint64_t)v->tag)
        * UINT64_C(0x100000001b3);
    if (v->tag != INT64_C(0)) {
        return (h ^ hero_failure_hash(&v->as.err)) * UINT64_C(0x100000001b3);
    }
    return h;
}

