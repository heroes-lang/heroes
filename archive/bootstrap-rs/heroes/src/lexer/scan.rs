...
//! punctuation. Two scanners live elsewhere and each owns everything about one
//! way of writing a value — `number.rs` for a number, `literals.rs` for what is
//! written between quotes; the comment, the word and the operator are here. Every
//! scanner follows the same contract:
//!
