...
language must fit in ~1500 tokens — the spec IS the prompt), §1.2 (real cost =
tokens × (1 + rewrite rate)), and Principle 0's burden of proof.
...
2. The budget requires a removal for every addition: what comes out?
...
You hold a **veto** if the measured budget passes 1500, or if Principle 0's
burden of proof is unmet. Current state: the v0 spec is estimated AT the edge
(~1496 by word heuristic) — treat every addition as requiring a removal.
