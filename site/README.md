...
| | The hero says **&ldquo;a programming language&rdquo;** above the name, because a visitor who has never heard of Heroes should not have to infer the category (author instruction 2026-08-17). It said *compiled* too until 2026-09-08, when the claim under the name started with *A compiled language* and the word would have been said twice in two lines; the category is still stated before the name, and *compiled* is the first thing the claim says. **Since 2026-09-10 the kicker is the one subtitle the language carries everywhere**, *A small compiled language*, by author decision among the five wordings then in use across the specification, the README and its banner, the share cards, the meta tags and the GitHub description: it is the line the specification opens with, and *small* is the adjective an instrument counts, the ceiling and the line count, where *simple*, the author's own word of 2026-09-09, was an opinion. With *compiled* back above the name, the claim under it no longer opens with *A compiled language*, so the word is said once; its closing line says *small enough to fit in a prompt*, the measured half of the old promise. The page title, the card alt and `llms.txt` say the same line. Its code panel shows **working code, never a diagnostic** — *&ldquo;opening a language's site on an error is ugly&rdquo;*, same date. The errors have their own page. In that panel the `bar` names the file, so the figure carries no second caption. |
...
many programs there are. Regenerating a card is a build of one HTML file and a
headless screenshot. **The generator was not in the repository until
2026-09-11**, when the home card had to be redrawn for the one subtitle and was
rebuilt from the pixels; the home card's template is `site/cards/home.html`
since then, English only, and the Italian card is a copy of it with its two
lines swapped at generation time, because `records/english` reads every `.html`
outside `src/html/it/` and would refuse an Italian source under `cards/`. The
command, from the repository root:
`"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless=new --hide-scrollbars --window-size=1200,630 --screenshot=site/public/images/card/home-en.png "file://$PWD/site/cards/home.html"`.
The other eighteen cards have no source in the repository.
...
- **The site says whose idea it was, and where it was the author's it says so**
  ...
  **The refusals page, `why/not.html`, is where the rule shows whole and in
  both directions** (2026-09-17): each no names the seat that cast the veto or
  the measurement that decided it, read from the sitting's record and never
  from memory, and the page says what the author's part was, ratifying every
  one, writing the three rules that bind them and wanting some from the first
  commit. The author asked for that credit in so many words, *"say that I had
  a hand in accepting those no's, push it a bit without lying shamelessly"*,
  and every sentence of it was checked against a dated record first.
...
postcard register, distilled from the files under `docs/records/book/beats/`,
not from the commit log, and deciding which milestones closed from `git tag`
and never from the chain's tag cell, which read `untagged` for three tagged
rows on 2026-09-17); re-check `why.html`'s objections against design.md's
current state; **re-check `why/not.html` against design.md Part 6 by hand**, a
struck or rewritten row is a postcard to rewrite and a new row is one to
consider, because the author declined a build check for that page
(2026-09-16); re-run every number; re-check each code block against its
`data-src` anchor.
