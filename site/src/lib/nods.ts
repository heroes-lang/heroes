...
/**
 * What the markup cannot know, in the order a reader meets the nods: the home
 * page, then the nav's own order, then document order inside a page. That order
 * is the walk through the site, so the leading-zero counter is something a
 * reader can use rather than decoration, and the last row is the nod this page
 * wears itself.
 *
 * Every record and year here was read from Wikipedia's own summary in the
 * session that wrote this file, and every `hear` is a YouTube id that was put
 * through the oEmbed endpoint, which 404s on a video that is gone and returns
 * the title and the channel for one that is live. Only the artist's channel and
 * the distributor's own art tracks are used: a third party's upload is somebody
 ...
 */
type Row = {
  /** The nod exactly as a heading spells it. */
  nod: string;
  ...
  recordIt?: string;
  year: number;
  /** Where to hear it, or the record itself when the nod names an album. */
  hear: string;
};
...
/** How many titles the site spends, which is what the prose claims. */
...
export function assertNodsMirrored(): number {
  ...
  const unspent = ROWS.filter((row) => !en.includes(row.nod));
  if (unspent.length > 0) {
    throw new Error(
      `${unspent.length} row(s) of the tracklist are spent by no nod: ` +
        `${unspent.map((row) => row.nod).join(', ')}.\n` +
        `  The markup is the record. Remove the row, or spend the title on a heading.\n`
    );
  }
  return en.length;
}
